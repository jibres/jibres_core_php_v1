...
class controller
{
	public static function routing()
	{
		...
		\dash\db::get('SELECT * FROM users ORDER BY users.id DESC Limit 1', null, false, 'master');
		$time['end_master_fuel'] = round(microtime(true) - $time['start_master_fuel'], 2). ' ms';

		$time['start_501_fuel'] = microtime(true);
		\dash\db::get('SELECT * FROM users ORDER BY users.id DESC Limit 1', null, false, '501');
		$time['end_501_fuel'] = round(microtime(true) - $time['start_501_fuel'], 2). ' ms';

		$time['start_400_fuel'] = microtime(true);
		\dash\db::get('SELECT * FROM users ORDER BY users.id DESC Limit 1', null, false, '400');

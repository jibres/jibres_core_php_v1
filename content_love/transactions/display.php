...
<div class="cbox fs12">
  <form method="get" action='<?php echo \dash\url::here(); ?>/transactions' data-action>
    <div class="input">
      <input type="search" name="q" placeholder='<?php echo T_("Search in :transactionss"); ?>' id="q" value="<?php echo \dash\validate::search_string(); ?>" <?php \dash\layout\autofocus::html() ?>  data-pass='submit' autocomplete='off'>
...
?>
 <div class="tblBbox">
  <table class="tbl1 v1">
    <thead>
      <tr>
      	<th data-sort=<?php echo a($sortLink, 'user_id', 'order'); ?>><a href="<?php echo a($sortLink, 'user_id', 'link'); ?>"><?php echo T_("User"); ?></a></th>
      	<th data-sort=<?php echo a($sortLink, 'title', 'order'); ?>><a href="<?php echo a($sortLink, 'title', 'link'); ?>"><?php echo T_("Title"); ?></a></th>
      	<th data-sort=<?php echo a($sortLink, 'plus', 'order'); ?>><a href="<?php echo a($sortLink, 'plus', 'link'); ?>"><i class="sf-plus-circle"></i></a></th>
      	<th data-sort=<?php echo a($sortLink, 'minus', 'order'); ?>><a href="<?php echo a($sortLink, 'minus', 'link'); ?>"><i class="sf-minus-circle"></i></a></th>
      	<th data-sort=<?php echo a($sortLink, 'budget', 'order'); ?>><a href="<?php echo a($sortLink, 'budget', 'link'); ?>"><?php echo T_("Budget"); ?></a></th>
    ...
    <tbody>
      ...
      <tr <?php if(isset($value['verify']) && $value['verify']) {?> class="positive" <?php } ?>>
		<td>
			...
			<?php if(isset($value['user_id']) && $value['user_id']) {?><a href="<?php echo \dash\url::here(); ?>/member/glance?id=<?php echo $value['user_id']; ?>"><?php echo \dash\fit::mobile(a($value, 'mobile')); ?></a><?php }else{ ?><small class="hidden"><?php echo T_("Anonymous"); ?></small><?php } ?>
		</td>

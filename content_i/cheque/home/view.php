...
class view
{
	public static function config()
	{
		...
		\dash\data::page_title(T_("check list"));


		\dash\data::page_pictogram('list');

		\dash\data::badge_link(\dash\url::this(). '/add');
		\dash\data::badge_text(T_('Add new check'));
		...
		{
			$args['check.status'] = \dash\request::get('status');
		}
		...
		{
			$args['i_check.title'] = \dash\request::get('title');
			$filterArgs['title']   = \dash\request::get('title');
		}

		if(\dash\request::get('firstserial'))
		{
			$args['i_check.firstserial'] = \dash\request::get('firstserial');
			$filterArgs['firstserial']   = \dash\request::get('firstserial');
		}
		if(\dash\request::get('number'))
		{
			$args['i_check.number'] = \dash\request::get('number');
			$filterArgs['number']   = \dash\request::get('number');
		}
		if(\dash\request::get('pagecount'))
		{
			$args['i_check.pagecount'] = \dash\request::get('pagecount');
			$filterArgs['pagecount']   = \dash\request::get('pagecount');
		}


		$sortLink  = \dash\app\sort::make_sortLink(\lib\app\check::$sort_field, \dash\url::this());
		$dataTable = \lib\app\check::list(\dash\request::get('q'), $args);

		\dash\data::sortLink($sortLink);
		\dash\data::dataTable($dataTable);

		if(isset($args['i_check.bank_id']) && isset($dataTable[0]['bank_title']))
		{

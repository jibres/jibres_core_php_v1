<?php
class TransStatic
{
 private function trans_static()
 {

	echo T_("Jibres");

	echo T_("Set home page number");

	echo T_(" ");



	echo T_('Monday');
	echo T_('Tuesday');
	echo T_('Wednesday');
	echo T_('Thursday');
	echo T_('Friday');
	echo T_('Saturday');
	echo T_('Sunday');


	echo T_("money");
	echo T_("toman");

	echo T_("sale");
	echo T_("buy");
	echo T_("presell");
	echo T_("lending");
	echo T_("backbuy");
	echo T_("backsell");
	echo T_("waste");

	echo T_("Sale");
	echo T_("Buy");
	echo T_("Presell");
	echo T_("Lending");
	echo T_("Backbuy");
	echo T_("Backsell");
	echo T_("Waste");

	echo T_("aThirdPartyPermissionChange");
	echo T_("reportDaily");
	echo T_("reportMonth");
	echo T_("reportView");
	echo T_("aThirdPartyEdit");
	echo T_("aThirdPartyAdd");
	echo T_("aStaffAdd");
	echo T_("aThirdPartyView");
	echo T_("aPermissionAddEdit");
	echo T_("aPermissionDelete");
	echo T_("aPermissionView");
	echo T_("settingBusinessEdit");
	echo T_("settingView");
	echo T_("aProductEdit");
	echo T_("aProductAdd");
	echo T_("aProductDelete");
	echo T_("aProductExport");
	echo T_("aProductImport");
	echo T_("aProductReport");
	echo T_("aProductList");
	echo T_("aFactorFishPrint");
	echo T_("aFactorAdd");
	echo T_("aFactorExport");
	echo T_("aFactorList");

	echo T_("Third party");
	echo T_("Store setting");
	echo T_("Report");

	echo T_("Customer");
	echo T_("Customers");
	echo T_("customer");
	echo T_("customers");
	echo T_("Staff");
	echo T_("Staffs");
	echo T_("staff");
	echo T_("staffs");
	echo T_("Supplier");
	echo T_("Suppliers");
	echo T_("supplier");
	echo T_("suppliers");
	echo T_("Thirdparty");
	echo T_("thirdparty");
	echo T_("Thirdparties");
	echo T_("thirdparties");

	echo T_("factorSaleView");
	echo T_("factorMySaleView");
	echo T_("factorSaleAdd");
	echo T_("factorSaleViewDetail");
	echo T_("factorBuyList");
	echo T_("factorMyBuyList");
	echo T_("factorBuyAdd");
	echo T_("factorBuyViewDetail");
	echo T_("productAdd");
	echo T_("productList");
	echo T_("productSummary");
	echo T_("productAdvanceSearchView");
	echo T_("productPriceAccess");
	echo T_("productFinalpriceAccess");
	echo T_("productDatemodifiedAccess");
	echo T_("productBuypriceAccess");
	echo T_("productDiscountAccess");
	echo T_("productProfitAccess");
	echo T_("productStockAccess");
	echo T_("productInitialbalanceEdit");
	echo T_("productEditGeneral");
	echo T_("productManagementView");
	echo T_("productStatusEdit");
	echo T_("productDelete");
	echo T_("productGlance");
	echo T_("productSite");
	echo T_("productThumbEdit");
	echo T_("productGalleryEdit");
	echo T_("productDescriptionEdit");
	echo T_("productPriceHistoryView");
	echo T_("productImport");
	echo T_("productExport");
	echo T_("productCategoryListAdd");
	echo T_("productCategoryListEdit");

	echo T_("productUnitListAdd");
	echo T_("productUnitListEdit");
	echo T_("reportListChartHours");
	echo T_("permissionList");
	echo T_("permissionAddEdit");
	echo T_("permissionDelete");
	echo T_("staffAccess");
	echo T_("supplierAccess");
	echo T_("customerAccess");
	echo T_("customerAdd");
	echo T_("supplierAdd");
	echo T_("staffAdd");
	echo T_("staffExport");
	echo T_("customerExport");
	echo T_("supplierExport");
	echo T_("settingBusinessEdit");
	echo T_("settingView");
	echo T_("reportDaily");
	echo T_("reportMonth");
	echo T_("reportView");

	echo T_("factorAccess");
	echo T_("factorEditAccess");
	echo T_("factorSaleList");
	echo T_("factorMySaleList");
	echo T_("productStock");
	echo T_("productReport");
	echo T_("productFactor");
	echo T_("productExport");
	echo T_("productCategoryListDelete");
	echo T_("productUnitListDelete");
	echo T_("thirdpartyBillingView");
	echo T_("settingBusinessEditPlan");
	echo T_("settingBusinessEditPos");
	echo T_("settingBusinessEditFactor");
	echo T_("settingBusinessEditFund");
	echo T_("settingBusinessEditInventory");
	echo T_("settingBusinessEditLogo");

	echo T_("trial");
	echo T_("Trial");
	echo T_("change");
	echo T_("continuation");
	echo T_("upgrade");
	echo T_("downgrade");
	echo T_("first_year");
	echo T_("set");
	echo T_("monthly");
	echo T_("yearly");

	echo T_("propertyAdd");
	echo T_("propertyDelete");
	echo T_("thirdpartyNoteDelete");
	echo T_("thirdpartySold");
	echo T_("thirdpartyLogView");
	echo T_("thirdpartyProfile");
	echo T_("thirdpartyTransaction");
	echo T_("productPropertyEdit");
	echo T_("productView");

	echo T_("categoryPropertyAddEdit");
	echo T_("categoryRemove");
	echo T_("categoryAdd");
	echo T_("categoryEdit");

	echo T_("ultimateFactor");
	echo T_("factorCount1000");
	echo T_("factorCount100");
	echo T_("ultimateProduct");
	echo T_("productCount1000");
	echo T_("productCount100");
	echo T_("ultimateThirdparty");
	echo T_("thirdpartyCount1000");
	echo T_("thirdpartyCount100");
	echo T_("ultimateFactorDetail");
	echo T_("factordetailCount100");
	echo T_("factordetailCount10");
	echo T_("Limitations");
	// ------------------------------------------------------------ Table comments
	echo T_("comments");            // Table comments
	echo T_("comment");             // comment
	echo T_("approved");            // Enum approved
	echo T_("unapproved");          // Enum unapproved
	echo T_("spam");                // Enum spam
	echo T_("deleted");             // Enum deleted
	echo T_("id");                  // id
	echo T_("post");                // post_id
	echo T_("author");              // comment_author
	echo T_("email");               // comment_email
	echo T_("url");                 // comment_url
	echo T_("content");             // comment_content
	echo T_("meta");                // comment_meta
	echo T_("status");              // comment_status
	echo T_("parent");              // comment_parent
	echo T_("user");                // user_id
	echo T_("visitor");             // visitor_id
	echo T_("modified");            // date_modified

	// ------------------------------------------------------------ Table hours
	echo T_("hours");               // Table hours
	echo T_("hour");                // hour
	echo T_("nothing");             // Enum nothing
	echo T_("base");                // Enum base
	echo T_("wplus");               // Enum wplus
	echo T_("wminus");              // Enum wminus
	echo T_("all");                 // Enum all
	echo T_("active");              // Enum active
	echo T_("awaiting");            // Enum awaiting
	echo T_("deactive");            // Enum deactive
	echo T_("removed");             // Enum removed
	echo T_("filter");              // Enum filter
	echo T_("date");                // hour_date
	echo T_("start");               // hour_start
	echo T_("end");                 // hour_end
	echo T_("diff");                // hour_diff
	echo T_("minus");               // hour_minus
	echo T_("plus");                // hour_plus
	echo T_("type");                // hour_type
	echo T_("accepted");            // hour_accepted
	echo T_("status");              // hour_status

	// ------------------------------------------------------------ Table logitems
	echo T_("logitems");            // Table logitems
	echo T_("logitem");             // logitem
	echo T_("critical");            // Enum critical
	echo T_("high");                // Enum high
	echo T_("medium");              // Enum medium
	echo T_("low");                 // Enum low
	echo T_("title");               // logitem_title
	echo T_("desc");                // logitem_desc
	echo T_("meta");                // logitem_meta
	echo T_("priority");            // logitem_priority

	// ------------------------------------------------------------ Table logs
	echo T_("logs");                // Table logs
	echo T_("log");                 // log
	echo T_("enable");              // Enum enable
	echo T_("disable");             // Enum disable
	echo T_("expire");              // Enum expire
	echo T_("deliver");             // Enum deliver
	echo T_("logitem");             // logitem_id
	echo T_("data");                // log_data
	echo T_("meta");                // log_meta
	echo T_("status");              // log_status
	echo T_("createdate");          // log_createdate

	// ------------------------------------------------------------ Table notifications
	echo T_("notifications");       // Table notifications
	echo T_("notification");        // notification
	echo T_("read");                // Enum read
	echo T_("unread");              // Enum unread
	echo T_("user sender");         // user_idsender
	echo T_("title");               // notification_title
	echo T_("content");             // notification_content
	echo T_("meta");                // notification_meta
	echo T_("url");                 // notification_url
	echo T_("status");              // notification_status

	// ------------------------------------------------------------ Table options
	echo T_("options");             // Table options
	echo T_("option");              // option
	echo T_("cat");                 // option_cat
	echo T_("key");                 // option_key
	echo T_("value");               // option_value
	echo T_("meta");                // option_meta
	echo T_("status");              // option_status

	// ------------------------------------------------------------ Table posts
	echo T_("posts");               // Table posts
	echo T_("post");                // post
	echo T_("open");                // Enum open
	echo T_("closed");              // Enum closed
	echo T_("publish");             // Enum publish
	echo T_("draft");               // Enum draft
	echo T_("schedule");            // Enum schedule
	echo T_("language");            // post_language
	echo T_("title");               // post_title
	echo T_("slug");                // post_slug
	echo T_("url");                 // post_url
	echo T_("content");             // post_content
	echo T_("meta");                // post_meta
	echo T_("type");                // post_type
	echo T_("comment");             // post_comment
	echo T_("count");               // post_count
	echo T_("order");               // post_order
	echo T_("status");              // post_status
	echo T_("parent");              // post_parent
	echo T_("publishdate");         // post_publishdate

	// ------------------------------------------------------------ Table terms
	echo T_("terms");               // Table terms
	echo T_("term");                // term
	echo T_("language");            // term_language
	echo T_("type");                // term_type
	echo T_("title");               // term_title
	echo T_("slug");                // term_slug
	echo T_("url");                 // term_url
	echo T_("desc");                // term_desc
	echo T_("meta");                // term_meta
	echo T_("parent");              // term_parent

	// ------------------------------------------------------------ Table termusages
	echo T_("termusages");          // Table termusages
	echo T_("termusage");           // termusage
	echo T_("posts");               // Enum posts
	echo T_("products");            // Enum products
	echo T_("attachments");         // Enum attachments
	echo T_("files");               // Enum files
	echo T_("comments");            // Enum comments
	echo T_("term");                // term_id
	echo T_("termusage");           // termusage_id
	echo T_("foreign");             // termusage_foreign
	echo T_("order");               // termusage_order

	// ------------------------------------------------------------ Table users
	echo T_("users");               // Table users
	echo T_("user");                // user
	echo T_("mobile");              // user_mobile
	echo T_("email");               // user_email
	echo T_("pass");                // user_pass
	echo T_("displayname");         // user_displayname
	echo T_("meta");                // user_meta
	echo T_("status");              // user_status
	echo T_("permission");          // user_permission
	echo T_("createdate");          // user_createdate




	// ------------------------------------------------------------ Static trans
	echo T_("barcode");


	// ------------------------------------------------------------ Enum in database
	echo T_("male");
	echo T_("female");
	echo T_("company");
	echo T_("rather");
	echo T_("active");
	echo T_("awaiting");
	echo T_("deactive");
	echo T_("removed");
	echo T_("filter");
	echo T_("unreachable");
	echo T_("single");
	echo T_("married");
	echo T_("product");
	echo T_("file");
	echo T_("service");
	echo T_("unset");
	echo T_("available");
	echo T_("unavailable");
	echo T_("soon");
	echo T_("discountinued");
	echo T_("deleted");
	echo T_("yes");
	echo T_("no");
	echo T_("enable");
	echo T_("disable");
	echo T_("leave");
	echo T_("spam");
	echo T_("delete");
	echo T_("sale");
	echo T_("buy");
	echo T_("presell");
	echo T_("lending");
	echo T_("backbuy");
	echo T_("backsell");
	echo T_("waste");
	echo T_("expire");
	echo T_("draft");
	echo T_("trash");
	echo T_("approved");
	echo T_("unapproved");
	echo T_("close");
	echo T_("answered");
	echo T_("publish");
	echo T_("block");
	echo T_("deliver");
	echo T_("cancel");
	echo T_("notif");
	echo T_("notifread");
	echo T_("notifexpire");
	echo T_("terminate");
	echo T_("changed");
	echo T_("logout");
	echo T_("ok");
	echo T_("failed");
	echo T_("other");
	echo T_("bot");
	echo T_("unknown");
	echo T_("used");
	echo T_("android");
	echo T_("ios");
	echo T_("api");
	echo T_("guest");
	echo T_("member");
	echo T_("appkey");
	echo T_("inline");
	echo T_("callback");
	echo T_("open");
	echo T_("closed");
	echo T_("schedule");
	echo T_("cat");
	echo T_("tag");
	echo T_("code");
	echo T_("term");
	echo T_("support_tag");
	echo T_("mag");
	echo T_("mag_tag");
	echo T_("help");
	echo T_("help_tag");
	echo T_("expired");
	echo T_("filtered");
	echo T_("blocked");
	echo T_("violence");
	echo T_("pornography");
	echo T_("posts");
	echo T_("products");
	echo T_("attachments");
	echo T_("files");
	echo T_("comments");
	echo T_("users");
	echo T_("tickets");
	echo T_("barcode1");
	echo T_("barcode2");
	echo T_("barcode3");
	echo T_("qrcode1");
	echo T_("qrcode2");
	echo T_("qrcode3");
	echo T_("rfid1");
	echo T_("rfid2");
	echo T_("rfid3");
	echo T_("gift");
	echo T_("prize");
	echo T_("transfer");
	echo T_("promo");
	echo T_("money");
	echo T_("request");
	echo T_("redirect");
	echo T_("pending");
	echo T_("error");
	echo T_("verify_request");
	echo T_("verify_error");
	echo T_("site");
	echo T_("telegram");
	echo T_("sms");
	echo T_("contact");
	echo T_("admincontact");
	echo T_("app");
	echo T_("success");
	echo T_("fail");
	echo T_("lock");
	echo T_("change");
	echo T_("continuation");
	echo T_("upgrade");
	echo T_("downgrade");
	echo T_("set");
	echo T_("auto");
	echo T_("monthly");
	echo T_("yearly");
	echo T_("running");
	echo T_("done");
	echo T_("Male");
	echo T_("Female");
	echo T_("Company");
	echo T_("Rather");
	echo T_("Active");
	echo T_("Awaiting");
	echo T_("Deactive");
	echo T_("Removed");
	echo T_("Filter");
	echo T_("Unreachable");
	echo T_("Single");
	echo T_("Married");
	echo T_("Product");
	echo T_("File");
	echo T_("Service");
	echo T_("Unset");
	echo T_("Available");
	echo T_("Unavailable");
	echo T_("Soon");
	echo T_("Discountinued");
	echo T_("Deleted");
	echo T_("Yes");
	echo T_("No");
	echo T_("Enable");
	echo T_("Disable");
	echo T_("Leave");
	echo T_("Spam");
	echo T_("Delete");
	echo T_("Sale");
	echo T_("Buy");
	echo T_("Presell");
	echo T_("Lending");
	echo T_("Backbuy");
	echo T_("Backsell");
	echo T_("Waste");
	echo T_("Expire");
	echo T_("Draft");
	echo T_("Trash");
	echo T_("Approved");
	echo T_("Unapproved");
	echo T_("Close");
	echo T_("Answered");
	echo T_("Publish");
	echo T_("Block");
	echo T_("Deliver");
	echo T_("Cancel");
	echo T_("Notif");
	echo T_("Notifread");
	echo T_("Notifexpire");
	echo T_("Terminate");
	echo T_("Changed");
	echo T_("Logout");
	echo T_("Ok");
	echo T_("Failed");
	echo T_("Other");
	echo T_("Bot");
	echo T_("Unknown");
	echo T_("Used");
	echo T_("Android");
	echo T_("Ios");
	echo T_("Api");
	echo T_("Guest");
	echo T_("Member");
	echo T_("Appkey");
	echo T_("Inline");
	echo T_("Callback");
	echo T_("Open");
	echo T_("Closed");
	echo T_("Schedule");
	echo T_("Cat");
	echo T_("Tag");
	echo T_("Code");
	echo T_("Term");
	echo T_("Support_tag");
	echo T_("Mag");
	echo T_("Mag_tag");
	echo T_("Help");
	echo T_("Help_tag");
	echo T_("Expired");
	echo T_("Filtered");
	echo T_("Blocked");
	echo T_("Violence");
	echo T_("Pornography");
	echo T_("Posts");
	echo T_("Products");
	echo T_("Attachments");
	echo T_("Files");
	echo T_("Comments");
	echo T_("Users");
	echo T_("Tickets");
	echo T_("Barcode1");
	echo T_("Barcode2");
	echo T_("Barcode3");
	echo T_("Qrcode1");
	echo T_("Qrcode2");
	echo T_("Qrcode3");
	echo T_("Rfid1");
	echo T_("Rfid2");
	echo T_("Rfid3");
	echo T_("Gift");
	echo T_("Prize");
	echo T_("Transfer");
	echo T_("Promo");
	echo T_("Money");
	echo T_("Request");
	echo T_("Redirect");
	echo T_("Pending");
	echo T_("Error");
	echo T_("Verify_request");
	echo T_("Verify_error");
	echo T_("Site");
	echo T_("Telegram");
	echo T_("Sms");
	echo T_("Contact");
	echo T_("Admincontact");
	echo T_("App");
	echo T_("Success");
	echo T_("Fail");
	echo T_("Lock");
	echo T_("Change");
	echo T_("Continuation");
	echo T_("Upgrade");
	echo T_("Downgrade");
	echo T_("Set");
	echo T_("Auto");
	echo T_("Monthly");
	echo T_("Yearly");
	echo T_("Running");
	echo T_("Done");



	echo T_('serverHold');
	echo T_('irnicReserved');
	echo T_('serverRenewProhibited');
	echo T_('serverDeleteProhibited');
	echo T_('irnicRegistrationPendingDomainCheck');
	echo T_('irnicRegistrationRejected');
	echo T_('pendingUpdate');
	echo T_('serverUpdateProhibited');
	echo T_('irnicUnapproved');
	echo T_('irnicApproved');
	echo T_('irnicQueued');
	echo T_('irnicRejected');
	echo T_('irnicLimited');
	echo T_('inactive');
	echo T_('pendingDelete');
	echo T_('pendingRenew');
	echo T_('pendingUpdate');
	echo T_('irnicRegistered');
	echo T_('irnicLocked');
	echo T_('irnicExpired');
	echo T_('irnicRegistrationApproved');
	echo T_('irnicRegistrationRejected');
	echo T_('irnicRegistrationPendingHolderCheck');
	echo T_('irnicRegistrationPendingDomainCheck');
	echo T_('irnicRegistrationDocRequired');
	echo T_('irnicRegistrationIsWithdrawn');
	echo T_('irnicRenewalApproved');
	echo T_('irnicRenewalRejected');
	echo T_('irnicRenewalPendingHolderCheck');
	echo T_('irnicRenewalPendingDomainCheck');
	echo T_('irnicRenewalDocRequired');
	echo T_('irnicRenewalIsWithdrawn');
	echo T_('irnicHolderChangeApproved');
	echo T_('irnicHolderChangeRejected');
	echo T_('irnicHolderChangePendingHolderCheck');
	echo T_('irnicHolderChangePendingDomainCheck');
	echo T_('irnicHolderChangeDocRequired');
	echo T_('irnicHolderChangeIsWithdrawn');
	echo T_('irnicDeletionApproved');
	echo T_('irnicDeletionRejected');
	echo T_('irnicDeletionPendingHolderCheck');
	echo T_('irnicDeletionPendingDomainCheck');
	echo T_('irnicDeletionDocRequired');
	echo T_('irnicDeletionIsWithdrawn');

	echo T_('period');
	echo T_('domain');
	echo T_('queue');
	echo T_('Queue');
	echo T_("Inprogress");
	echo T_("inprogress");
	echo T_("postcode");
	echo T_("Postcode");
	echo T_("Fullname");
	echo T_("fullname");
	echo T_("Debtor-creditor");
	echo T_("Balance sheet");
	echo T_("Harmful profit");
	echo T_("Details");
	echo T_("Detail");


	echo T_("sales costs");
	echo T_("financial costs");
	echo T_("administrative costs");
	echo T_("general expenses");
	echo T_("intangible fixed assets");


	echo T_("net sales");
	echo T_("accumulated profit");
	echo T_("orders and prepayments");
	echo T_("short term investments");
	echo T_("wealth");
	echo T_("other non-operating expenses");
	echo T_("other operating costs");
	echo T_("other operating income");
	echo T_("other accounts and documents receivable");
	echo T_("other accounts and documents payable");
	echo T_("save employee end-of-service benefits");
	echo T_("save taxes");
	echo T_("tangible fixed assets");
	echo T_("accounts receivable and commercial documents");
	echo T_("business accounts and documents payable");
	echo T_("receivables");
	echo T_("prepayments");
	echo T_("long-term accounts and documents");
	echo T_("cash balance");
	echo T_("received financial facilities");
	echo T_("administrative costs");
	echo T_("sales costs");
	echo T_("financial costs");
	echo T_("general expenses");
	echo T_("intangible fixed assets");


	echo T_("Net sales");
	echo T_("Accumulated profit");
	echo T_("Orders and prepayments");
	echo T_("Short term investments");
	echo T_("Wealth");
	echo T_("Other non-operating expenses");
	echo T_("Other operating costs");
	echo T_("Other operating income");
	echo T_("Other accounts and documents receivable");
	echo T_("Other accounts and documents payable");
	echo T_("Save employee end-of-service benefits");
	echo T_("Save taxes");
	echo T_("Tangible fixed assets");
	echo T_("Accounts receivable and commercial documents");
	echo T_("Business accounts and documents payable");
	echo T_("Rceivables");
	echo T_("Prepayments");
	echo T_("Long-term accounts and documents");
	echo T_("Cash balance");
	echo T_("Received financial facilities");
	echo T_("Administrative costs");
	echo T_("Sales costs");
	echo T_("Financial costs");
	echo T_("General expenses");
	echo T_("Intangible fixed assets");

	echo T_("temp");
	echo T_("Temp");

	echo T_('Jibres is not just an online accounting software; We try to create the best financial platform that has everything you need to sale and manage your financial life.');
	echo T_('Sell & Enjoy');


	echo T_("Iran");
	echo T_("Afghanistan");
	echo T_("Åland");
	echo T_("Albania");
	echo T_("Algeria");
	echo T_("American Samoa");
	echo T_("Andorra");
	echo T_("Angola");
	echo T_("Anguilla");
	echo T_("Antarctica");
	echo T_("Antigua And Barbuda");
	echo T_("Argentina");
	echo T_("Armenia");
	echo T_("Aruba");
	echo T_("Australia");
	echo T_("Austria");
	echo T_("Azerbaijan");
	echo T_("Bahamas");
	echo T_("Bahrain");
	echo T_("Bangladesh");
	echo T_("Barbados");
	echo T_("Belarus");
	echo T_("Belgium");
	echo T_("Belize");
	echo T_("Benin");
	echo T_("Bermuda");
	echo T_("Bhutan");
	echo T_("Bolivia");
	echo T_("Bonaire");
	echo T_("Bosnia And Herzegovina");
	echo T_("Botswana");
	echo T_("Bouvet Island");
	echo T_("Brazil");
	echo T_("British Indian Ocean Territory");
	echo T_("British Virgin Islands");
	echo T_("Brunei");
	echo T_("Bulgaria");
	echo T_("Burkina Faso");
	echo T_("Burundi");
	echo T_("Cambodia");
	echo T_("Cameroon");
	echo T_("Canada");
	echo T_("Cape Verde");
	echo T_("Cayman Islands");
	echo T_("Central African Republic");
	echo T_("Chad");
	echo T_("Chile");
	echo T_("China");
	echo T_("Christmas Island");
	echo T_("Cocos (Keeling) Islands");
	echo T_("Colombia");
	echo T_("Comoros");
	echo T_("Cook Islands");
	echo T_("Costa Rica");
	echo T_("Croatia");
	echo T_("Cuba");
	echo T_("Curacao");
	echo T_("Cyprus");
	echo T_("Czech Republic");
	echo T_("Democratic Republic Of The Congo");
	echo T_("Denmark");
	echo T_("Djibouti");
	echo T_("Dominica");
	echo T_("Dominican Republic");
	echo T_("East Timor");
	echo T_("Ecuador");
	echo T_("Egypt");
	echo T_("El Salvador");
	echo T_("Equatorial Guinea");
	echo T_("Eritrea");
	echo T_("Estonia");
	echo T_("Ethiopia");
	echo T_("Falkland Islands");
	echo T_("Faroe Islands");
	echo T_("Fiji");
	echo T_("Finland");
	echo T_("France");
	echo T_("French Guiana");
	echo T_("French Polynesia");
	echo T_("French Southern Territories");
	echo T_("Gabon");
	echo T_("Gambia");
	echo T_("Georgia");
	echo T_("Germany");
	echo T_("Ghana");
	echo T_("Gibraltar");
	echo T_("Greece");
	echo T_("Greenland");
	echo T_("Grenada");
	echo T_("Guadeloupe");
	echo T_("Guam");
	echo T_("Guatemala");
	echo T_("Guernsey");
	echo T_("Guinea");
	echo T_("Guinea-bissau");
	echo T_("Guyana");
	echo T_("Haiti");
	echo T_("Heard Island And Mcdonald Islands");
	echo T_("Honduras");
	echo T_("Hong Kong");
	echo T_("Hungary");
	echo T_("Iceland");
	echo T_("India");
	echo T_("Indonesia");
	echo T_("Iraq");
	echo T_("Ireland");
	echo T_("Isle Of Man");
	echo T_("Italy");
	echo T_("Ivory Coast");
	echo T_("Jamaica");
	echo T_("Japan");
	echo T_("Jersey");
	echo T_("Jordan");
	echo T_("Kazakhstan");
	echo T_("Kenya");
	echo T_("Kiribati");
	echo T_("Kosovo");
	echo T_("Kuwait");
	echo T_("Kyrgyzstan");
	echo T_("Laos");
	echo T_("Latvia");
	echo T_("Lebanon");
	echo T_("Lesotho");
	echo T_("Liberia");
	echo T_("Libya");
	echo T_("Liechtenstein");
	echo T_("Lithuania");
	echo T_("Luxembourg");
	echo T_("Macau");
	echo T_("Macedonia");
	echo T_("Madagascar");
	echo T_("Malawi");
	echo T_("Malaysia");
	echo T_("Maldives");
	echo T_("Mali");
	echo T_("Malta");
	echo T_("Marshall Islands");
	echo T_("Martinique");
	echo T_("Mauritania");
	echo T_("Mauritius");
	echo T_("Mayotte");
	echo T_("Mexico");
	echo T_("Micronesia");
	echo T_("Moldova");
	echo T_("Monaco");
	echo T_("Mongolia");
	echo T_("Montenegro");
	echo T_("Montserrat");
	echo T_("Morocco");
	echo T_("Mozambique");
	echo T_("Myanmar (Burma)");
	echo T_("Namibia");
	echo T_("Nauru");
	echo T_("Nepal");
	echo T_("Netherlands");
	echo T_("New Caledonia");
	echo T_("New Zealand");
	echo T_("Nicaragua");
	echo T_("Niger");
	echo T_("Nigeria");
	echo T_("Niue");
	echo T_("Norfolk Island");
	echo T_("North Korea");
	echo T_("Northern Mariana Islands");
	echo T_("Norway");
	echo T_("Oman");
	echo T_("Pakistan");
	echo T_("Palau");
	echo T_("Palestine");
	echo T_("Panama");
	echo T_("Papua New Guinea");
	echo T_("Paraguay");
	echo T_("Peru");
	echo T_("Philippines");
	echo T_("Pitcairn Islands");
	echo T_("Poland");
	echo T_("Portugal");
	echo T_("Puerto Rico");
	echo T_("Qatar");
	echo T_("Republic Of The Congo");
	echo T_("Réunion");
	echo T_("Romania");
	echo T_("Russia");
	echo T_("Rwanda");
	echo T_("Saint Barthélemy");
	echo T_("Saint Helena");
	echo T_("Saint Kitts And Nevis");
	echo T_("Saint Lucia");
	echo T_("Saint Martin");
	echo T_("Saint Pierre And Miquelon");
	echo T_("Saint Vincent And The Grenadines");
	echo T_("Samoa");
	echo T_("San Marino");
	echo T_("São Tomé And Príncipe");
	echo T_("Saudi Arabia");
	echo T_("Senegal");
	echo T_("Serbia");
	echo T_("Seychelles");
	echo T_("Sierra Leone");
	echo T_("Singapore");
	echo T_("Sint Maarten");
	echo T_("Slovakia");
	echo T_("Slovenia");
	echo T_("Solomon Islands");
	echo T_("Somalia");
	echo T_("South Africa");
	echo T_("South Georgia And The South Sandwich Islands");
	echo T_("South Korea");
	echo T_("South Sudan");
	echo T_("Spain");
	echo T_("Sri Lanka");
	echo T_("Sudan");
	echo T_("Suriname");
	echo T_("Svalbard And Jan Mayen");
	echo T_("Swaziland");
	echo T_("Sweden");
	echo T_("Switzerland");
	echo T_("Syria");
	echo T_("Taiwan");
	echo T_("Tajikistan");
	echo T_("Tanzania");
	echo T_("Thailand");
	echo T_("Togo");
	echo T_("Tokelau");
	echo T_("Tonga");
	echo T_("Trinidad And Tobago");
	echo T_("Tunisia");
	echo T_("Turkey");
	echo T_("Turkmenistan");
	echo T_("Turks And Caicos Islands");
	echo T_("Tuvalu");
	echo T_("U.s. Minor Outlying Islands'");
	echo T_("U.s. Virgin Islands");
	echo T_("Uganda");
	echo T_("Ukraine");
	echo T_("United Arab Emirates");
	echo T_("United Kingdom");
	echo T_("United States");
	echo T_("Uruguay");
	echo T_("Uzbekistan");
	echo T_("Vanuatu");
	echo T_("Vatican City");
	echo T_("Venezuela");
	echo T_("Vietnam");
	echo T_("Wallis And Futuna");
	echo T_("Western Sahara");
	echo T_("Yemen");
	echo T_("Zambia");
	echo T_("Zimbabwechoe");


	echo T_("Archive");
	echo T_("archive");


	echo T_("Privacy");
	echo T_("privacy");

	echo T_("Public");
	echo T_("public");

	echo T_("Private");
	echo T_("private");

	echo T_('saleorder');
	echo T_('Saleorder');


	echo T_('sended');
	echo T_('Sended');

	echo T_("Irnic");
	echo T_("Onlinenic");

	echo T_("irnic");
	echo T_("onlinenic");

	echo T_("Cost");
	echo T_("Income");
	echo T_("cost");
	echo T_("income");


	echo T_('register');
	echo T_('pending');
	echo T_('sending');
	echo T_('expired');
	echo T_('moneylow');
	echo T_('unknown');
	echo T_('send');
	echo T_('sended');
	echo T_('delivered');
	echo T_('queue');
	echo T_('failed');
	echo T_('undelivered');
	echo T_('cancel');
	echo T_('block');
	echo T_('other');


	echo T_('Register');
	echo T_('Pending');
	echo T_('Sending');
	echo T_('Expired');
	echo T_('Moneylow');
	echo T_('Unknown');
	echo T_('Send');
	echo T_('Sended');
	echo T_('Delivered');
	echo T_('Queue');
	echo T_('Failed');
	echo T_('Undelivered');
	echo T_('Cancel');
	echo T_('Block');
	echo T_('Other');
	echo T_('accept');


	// currency
	echo T_('Toman');
	echo T_('Rial');
	echo T_('US Dollar');
	echo T_('Euro');
	echo T_('British Pound Sterling');
	echo T_('Japanese Yen');
	echo T_('Canadian Dollar');
	echo T_('United Arab Emirates Dirham');
	echo T_('Afghan Afghani');
	echo T_('Albanian Lek');
	echo T_('Armenian Dram');
	echo T_('Argentine Peso');
	echo T_('Australian Dollar');
	echo T_('Azerbaijani Manat');
	echo T_('Bosnia-Herzegovina Convertible Mark');
	echo T_('Bangladeshi Taka');
	echo T_('Bulgarian Lev');
	echo T_('Bahraini Dinar');
	echo T_('Burundian Franc');
	echo T_('Brunei Dollar');
	echo T_('Bolivian Boliviano');
	echo T_('Brazilian Real');
	echo T_('Botswanan Pula');
	echo T_('Belarusian Ruble');
	echo T_('Belize Dollar');
	echo T_('Congolese Franc');
	echo T_('Swiss Franc');
	echo T_('Chilean Peso');
	echo T_('Chinese Yuan');
	echo T_('Colombian Peso');
	echo T_('Costa Rican Colón');
	echo T_('Cape Verdean Escudo');
	echo T_('Czech Republic Koruna');
	echo T_('Djiboutian Franc');
	echo T_('Danish Krone');
	echo T_('Dominican Peso');
	echo T_('Algerian Dinar');
	echo T_('Estonian Kroon');
	echo T_('Egyptian Pound');
	echo T_('Eritrean Nakfa');
	echo T_('Ethiopian Birr');
	echo T_('Georgian Lari');
	echo T_('Ghanaian Cedi');
	echo T_('Guinean Franc');
	echo T_('Guatemalan Quetzal');
	echo T_('Hong Kong Dollar');
	echo T_('Honduran Lempira');
	echo T_('Croatian Kuna');
	echo T_('Hungarian Forint');
	echo T_('Indonesian Rupiah');
	echo T_('Israeli New Sheqel');
	echo T_('Indian Rupee');
	echo T_('Iraqi Dinar');
	echo T_('Thousand Toman');
	echo T_('Thousand Rial');
	echo T_('Icelandic Króna');
	echo T_('Jamaican Dollar');
	echo T_('Jordanian Dinar');
	echo T_('Kenyan Shilling');
	echo T_('Cambodian Riel');
	echo T_('Comorian Franc');
	echo T_('South Korean Won');
	echo T_('Kuwaiti Dinar');
	echo T_('Kazakhstani Tenge');
	echo T_('Lebanese Pound');
	echo T_('Sri Lankan Rupee');
	echo T_('Lithuanian Litas');
	echo T_('Latvian Lats');
	echo T_('Libyan Dinar');
	echo T_('Moroccan Dirham');
	echo T_('Moldovan Leu');
	echo T_('Malagasy Ariary');
	echo T_('Macedonian Denar');
	echo T_('Myanma Kyat');
	echo T_('Macanese Pataca');
	echo T_('Mauritian Rupee');
	echo T_('Mexican Peso');
	echo T_('Malaysian Ringgit');
	echo T_('Mozambican Metical');
	echo T_('Namibian Dollar');
	echo T_('Nigerian Naira');
	echo T_('Nicaraguan Córdoba');
	echo T_('Norwegian Krone');
	echo T_('Nepalese Rupee');
	echo T_('New Zealand Dollar');
	echo T_('Omani Rial');
	echo T_('Panamanian Balboa');
	echo T_('Peruvian Nuevo Sol');
	echo T_('Philippine Peso');
	echo T_('Pakistani Rupee');
	echo T_('Polish Zloty');
	echo T_('Paraguayan Guarani');
	echo T_('Qatari Rial');
	echo T_('Romanian Leu');
	echo T_('Serbian Dinar');
	echo T_('Russian Ruble');
	echo T_('Rwandan Franc');
	echo T_('Saudi Riyal');
	echo T_('Sudanese Pound');
	echo T_('Swedish Krona');
	echo T_('Singapore Dollar');
	echo T_('Somali Shilling');
	echo T_('Syrian Pound');
	echo T_('Thai Baht');
	echo T_('Tunisian Dinar');
	echo T_('Tongan Paʻanga');
	echo T_('Turkish Lira');
	echo T_('Trinidad and Tobago Dollar');
	echo T_('New Taiwan Dollar');
	echo T_('Tanzanian Shilling');
	echo T_('Ukrainian Hryvnia');
	echo T_('Ugandan Shilling');
	echo T_('Uruguayan Peso');
	echo T_('Uzbekistan Som');
	echo T_('Venezuelan Bolívar');
	echo T_('Vietnamese Dong');
	echo T_('CFA Franc BEAC');
	echo T_('CFA Franc BCEAO');
	echo T_('Yemeni Rial');
	echo T_('South African Rand');
	echo T_('Zambian Kwacha');
	echo T_('Resend');
	echo T_('resend');
 }
}
?>
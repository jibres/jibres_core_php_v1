
#---------------------------------------------------------------------- /a
---2017-10-24 08:30:58
	---0.00037503242492676s		---0ms
	SELECT * FROM sessions WHERE `code` = '$2y$07$wII02C.rM4Dip.P1hs2PjO8w1P.lmbXTVahj/.gdHKlk2VW11c00a' AND `status` = 'active' LIMIT 1

#---------------------------------------------------------------------- /a
---2017-10-24 08:30:58
	---0.0045788288116455s		---5ms
	UPDATE sessions SET sessions.count = sessions.count + 1 WHERE code = '$2y$07$wII02C.rM4Dip.P1hs2PjO8w1P.lmbXTVahj/.gdHKlk2VW11c00a'

#---------------------------------------------------------------------- /a
---2017-10-24 08:30:58
	---0.00034713745117188s		---0ms
	SELECT * FROM users WHERE `id` = 1 LIMIT 1

#---------------------------------------------------------------------- /a
---2017-10-24 08:30:58
	---0.0002589225769043s		---0ms
	SELECT * FROM agents WHERE `agent` = 'Mozilla%2F5.0+%28X11%3B+Linux+x86_64%29+AppleWebKit%2F537.36+%28KHTML%2C+like+Gecko%29+Chrome%2F55.0.2883.87+Safari%2F537.36' LIMIT 1

#---------------------------------------------------------------------- /a
---2017-10-24 08:30:58
	---0.0022120475769043s		---2ms
	UPDATE sessions SET sessions.count = sessions.count + 1 WHERE code = '$2y$07$wII02C.rM4Dip.P1hs2PjO8w1P.lmbXTVahj/.gdHKlk2VW11c00a'

#---------------------------------------------------------------------- /a
---2017-10-24 08:30:58
	---0.00099492073059082s		---1ms
	SELECT COUNT(*) AS `count` FROM `stores` WHERE `creator` = 1

#---------------------------------------------------------------------- /a
---2017-10-24 08:30:58
	---0.00029206275939941s		---0ms
	SELECT * FROM `stores` WHERE `creator` = 1 ORDER BY `stores`.`id` DESC LIMIT 0, 10 -- stores::search() -- ["stores","1",{"creator":"1"}]

#---------------------------------------------------------------------- /a
---2017-10-24 08:31:08
	---0.00052618980407715s		---1ms
	SELECT COUNT(*) AS `count` FROM `stores` WHERE `creator` = 1

#---------------------------------------------------------------------- /a
---2017-10-24 08:31:08
	---0.00037193298339844s		---0ms
	SELECT * FROM `stores` WHERE `creator` = 1 ORDER BY `stores`.`id` DESC LIMIT 0, 10 -- stores::search() -- ["stores","1",{"creator":"1"}]

#---------------------------------------------------------------------- /a
---2017-10-24 08:31:11
	---0.00032687187194824s		---0ms
	SELECT COUNT(*) AS `count` FROM `stores` WHERE `creator` = 1

#---------------------------------------------------------------------- /a
---2017-10-24 08:31:11
	---0.00025200843811035s		---0ms
	SELECT * FROM `stores` WHERE `creator` = 1 ORDER BY `stores`.`id` DESC LIMIT 0, 10 -- stores::search() -- ["stores","1",{"creator":"1"}]

#---------------------------------------------------------------------- /
---2017-10-24 13:48:06
	---0.0058920383453369s		---6ms
	SELECT * FROM sessions WHERE `code` = '$2y$07$wII02C.rM4Dip.P1hs2PjO8w1P.lmbXTVahj/.gdHKlk2VW11c00a' AND `status` = 'active' LIMIT 1

#---------------------------------------------------------------------- /
---2017-10-24 13:48:06
	---0.0061728954315186s		---6ms
	UPDATE sessions SET sessions.count = sessions.count + 1 WHERE code = '$2y$07$wII02C.rM4Dip.P1hs2PjO8w1P.lmbXTVahj/.gdHKlk2VW11c00a'

#---------------------------------------------------------------------- /
---2017-10-24 13:48:06
	---0.0016250610351562s		---2ms
	SELECT * FROM users WHERE `id` = 1 LIMIT 1

#---------------------------------------------------------------------- /
---2017-10-24 13:48:06
	---0.00034189224243164s		---0ms
	SELECT * FROM agents WHERE `agent` = 'Mozilla%2F5.0+%28X11%3B+Linux+x86_64%29+AppleWebKit%2F537.36+%28KHTML%2C+like+Gecko%29+Chrome%2F55.0.2883.87+Safari%2F537.36' LIMIT 1

#---------------------------------------------------------------------- /
---2017-10-24 13:48:06
	---0.0034010410308838s		---3ms
	UPDATE sessions SET sessions.count = sessions.count + 1 WHERE code = '$2y$07$wII02C.rM4Dip.P1hs2PjO8w1P.lmbXTVahj/.gdHKlk2VW11c00a'

#---------------------------------------------------------------------- /a
---2017-10-24 13:51:45
	---0.0019159317016602s		---2ms
	SELECT COUNT(*) AS `count` FROM `stores` WHERE `creator` = 1

#---------------------------------------------------------------------- /a
---2017-10-24 13:51:45
	---0.00071883201599121s		---1ms
	SELECT * FROM `stores` WHERE `creator` = 1 ORDER BY `stores`.`id` DESC LIMIT 0, 10 -- stores::search() -- ["stores","1",{"creator":"1"}]

#---------------------------------------------------------------------- /enter
---2017-10-24 16:00:29
	---0.00032997131347656s		---0ms
	SELECT * FROM users WHERE users.email = 'rezaa' OR users.username = 'rezaa' LIMIT 1

#---------------------------------------------------------------------- /enter
---2017-10-24 16:00:35
	---0.00043010711669922s		---0ms
	SELECT * FROM users WHERE users.email = '09109610612' OR users.username = '09109610612' OR users.mobile = '989109610612' LIMIT 1

#---------------------------------------------------------------------- /enter/pass
---2017-10-24 16:00:38
	---0.00036001205444336s		---0ms
	SELECT * FROM users WHERE `id` = '1' LIMIT 1

#---------------------------------------------------------------------- /enter/pass
---2017-10-24 16:00:38
	---0.0002589225769043s		---0ms
	SELECT * FROM agents WHERE `agent` = 'Mozilla%2F5.0+%28X11%3B+Linux+x86_64%29+AppleWebKit%2F537.36+%28KHTML%2C+like+Gecko%29+Chrome%2F55.0.2883.87+Safari%2F537.36' LIMIT 1

#---------------------------------------------------------------------- /enter/pass
---2017-10-24 16:00:38
	---0.00032591819763184s		---0ms
	SELECT * FROM sessions WHERE `ip` = '2130706433' AND `agent_id` = 1 AND `user_id` = '1' AND `status` = 'active' LIMIT 1

#---------------------------------------------------------------------- /enter/pass
---2017-10-24 16:00:38
	---0.0064511299133301s		---6ms
	UPDATE sessions SET sessions.count = sessions.count + 1 WHERE code = '$2y$07$wII02C.rM4Dip.P1hs2PjO8w1P.lmbXTVahj/.gdHKlk2VW11c00a'

#---------------------------------------------------------------------- /enter/pass
---2017-10-24 16:00:38
	---0.00030899047851562s		---0ms
	SELECT * FROM users WHERE `id` = '1' LIMIT 1

#---------------------------------------------------------------------- /a
---2017-10-24 16:00:38
	---0.00040602684020996s		---0ms
	SELECT COUNT(*) AS `count` FROM `stores` WHERE `creator` = 1

#---------------------------------------------------------------------- /a
---2017-10-24 16:00:38
	---0.00031590461730957s		---0ms
	SELECT * FROM `stores` WHERE `creator` = 1 ORDER BY `stores`.`id` DESC LIMIT 0, 10 -- stores::search() -- ["stores","1",{"creator":"1"}]

#---------------------------------------------------------------------- /a
---2017-10-24 16:00:47
	---0.0004570484161377s		---0ms
	SELECT COUNT(*) AS `count` FROM `stores` WHERE `creator` = 1

#---------------------------------------------------------------------- /a
---2017-10-24 16:00:47
	---0.00034308433532715s		---0ms
	SELECT * FROM `stores` WHERE `creator` = 1 ORDER BY `stores`.`id` DESC LIMIT 0, 10 -- stores::search() -- ["stores","1",{"creator":"1"}]

	...
	private static function check($_option = [])
	{
		...
		if(\lib\app::isset_request('title'))
		{
			...
			if(mb_strlen($title) >= 500)
			{
				...
				if($_option['debug']) \lib\debug::error(T_("Product title must be less than 500 character"), 'title');
				return false;
			}
		}
		...
		if($name && mb_strlen($name) >= 500)
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Product name must be less than 500 character"), 'name');
			return false;
		}
		...
		if($cat && mb_strlen($cat) >= 200)
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Product cat must be less than 200 character"), 'cat');
			return false;
		}
		...
		if($company && mb_strlen($company) >= 200)
		{
			...
			if($_option['debug']) \lib\debug::error(T_("String of product company is too large"), 'company');
			return false;
		}
		...
		// 	if($_option['debug']) \lib\debug::error(T_("Shortcode must be a number"), 'shortcode');
		// 	return false;
		// }
		...
		// 	if($_option['debug']) \lib\debug::error(T_("Value of shortcode is out of rage"), 'shortcode');
		// 	return false;
		// }
		...
		if($unit && mb_strlen($unit) >= 100)
		{
			...
			if($_option['debug']) \lib\debug::error(T_("String of product unit is too large"), 'unit');
			return false;
		}
		...
		if($barcode && mb_strlen($barcode) >= 100)
		{
			...
			if($_option['debug']) \lib\debug::error(T_("String of product barcode is too large"), 'barcode');
			return false;
		}
		...
		if($barcode2 && mb_strlen($barcode2) >= 100)
		{
			...
			if($_option['debug']) \lib\debug::error(T_("String of product barcode2 is too large"), 'barcode2');
			return false;
		}
		...
		if($code && mb_strlen($code) >= 200)
		{
			...
			if($_option['debug']) \lib\debug::error(T_("String of product code is too large"), 'code');
			return false;
		}
		...
		if($buyprice && !is_numeric($buyprice))
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of buyprice muset be a number"), 'buyprice');
			return false;
		}
		...
		if(floatval($buyprice) >= 1E+20 || floatval($buyprice) < 0)
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of buyprice is out of rage"), 'buyprice');
			return false;
		}
		...
		if($price && !is_numeric($price))
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of price muset be a number"), 'price');
			return false;
		}
		...
		if(floatval($price) >= 1E+20 || floatval($price) < 0)
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of price is out of rage"), 'price');
			return false;
		}
		...
		if($discount && !is_numeric($discount))
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of discount muset be a number"), 'discount');
			return false;
		}
		...
		if($discount && abs(floatval($discount)) >= 1E+10)
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of discount is out of rage"), 'discount');
			return false;
		}
		...
		if($initialbalance && !is_numeric($initialbalance))
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of initialbalance muset be a number"), 'initialbalance');
			return false;
		}
		...
		if(floatval($initialbalance) >= 1E+10 || floatval($initialbalance) < 0)
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of initialbalance is out of rage"), 'initialbalance');
			return false;
		}
		...
		if($minstock && !is_numeric($minstock))
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of minstock muset be a number"), 'minstock');
			return false;
		}
		...
		if(floatval($minstock) >= 1E+10 || floatval($minstock) < 0)
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of minstock is out of rage"), 'minstock');
			return false;
		}
		...
		if($maxstock && !is_numeric($maxstock))
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of maxstock muset be a number"), 'maxstock');
			return false;
		}
		...
		if(floatval($maxstock) >= 1E+10 || floatval($maxstock) < 0)
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of maxstock is out of rage"), 'maxstock');
			return false;
		}
		...
		if($status && !in_array($status, ['unset','available','unavailable','soon','discountinued']))
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Product status is incorrect"), 'status');
			return false;
		}
		...
		// 	if($_option['debug']) \lib\debug::error(T_("Value of sold is out of rage"), 'sold');
		// 	return false;
		// }
		...
		if($stock && !is_numeric($stock))
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of stock muset be a number"), 'stock');
			return false;
		}
		...
		if(abs(floatval($stock)) >= 1E+20)
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of stock is out of rage"), 'stock');
			return false;
		}
		...
		if($thumb && !is_string($thumb))
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of thumb is out of rage"), 'thumb');
			return false;
		}
		...
		if($carton && !is_numeric($carton))
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of carton muset be a number"), 'carton');
			return false;
		}
		...
		if(floatval($carton) >= 1E+10 || floatval($carton) < 0)
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of carton is out of rage"), 'carton');
			return false;
		}
		...
		if($desc && mb_strlen($desc))
		{
			...
			if($_option['debug']) \lib\debug::error(T_("Value of desc is out of rage"), 'desc');
			return false;
		}
		...
		if($finalprice < 0)
		{
			\lib\debug::error(T_("Final price is less than 0"), 'price');
			return false;
		}

		if($finalprice < floatval($buyprice))
		{
			\lib\debug::warn(T_("Final price less than buyprice!"));
		}

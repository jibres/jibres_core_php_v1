...
class unit
{
	...
	public static function get($_id)
	{
		...
		$result = \lib\db\unit::get(['id' => $id, 'limit' => 1]);
		$temp = [];
		if(is_array($result))
		{
	...
	public static function check($_id = null)
	{
		...
		if($parent1)
		{
			...
			$check = \lib\db\unit::get(['user_id' => \dash\user::id(), 'id' => $parent1, 'limit' => 1]);
			if(!isset($check['id']))
			{
				\dash\notif::error(T_("Invalid parent"), 'parent');
				return false;
		...
		$check_duplicate = \lib\db\unit::get($check_duplicate);
		...
		if(isset($check_duplicate['id']))
		{
			...
			{
				\dash\notif::error(T_("Duplicate unit title"), 'title');
				return false;
			}
		}
	...
	public static function add($_args, $_option = [])
	{
		...
		$unit = \lib\db\unit::insert($args);

		if(!$unit)
		{
			\dash\log::set('noWayToAddCategory');
			\dash\notif::error(T_("No way to insert unit"));
			return false;
		}

		$return['cat_id'] = $unit;

		\dash\log::set('iAddCategory', ['code' => $unit]);
		return $return;
	}
	...
	public static function list($_string = null, $_args = [])
	{
		...
		$result = \lib\db\unit::search($_string, $option);
	...
	public static function edit($_args, $_id)
	{
		...
		if(!$id)
		{
			\dash\notif::error(T_("Can not access to edit unit"), 'unit');
			return false;
		}

...
<form method="post" autocomplete="off" id="formEditPost">
  ...
  $gallery = \dash\get::index(\dash\data::dataRow_meta(), 'gallery', 'files');
  if(!is_array($gallery))
  {
    $gallery = [];
  }
  ...
  <div class="box">
    <div class="pad1">
      <?php if(is_array($gallery) && count($gallery) > 10) {?>
                ...
                <?php foreach ($gallery as $key => $value) {?>
                  <div class="fileItem" data-removeElement data-type='<?php echo \dash\get::index($value, 'type'); ?>'>
                    <?php if(\dash\get::index($value, 'type') === 'video') {?>
                      ...
                      <?php } else { ?>
                        <img src="<?php echo \dash\get::index($value, 'path'); ?>" alt="<?php echo \dash\get::index($dataRow, 'title'); ?>">
                        <?php if($key === 0) {?>
                        <?php }else{ ?>

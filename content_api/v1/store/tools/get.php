...
trait get
{
	 ...
	 */
	public function ready_store($_data, $_options = [])
	{
		$default_options =
		[
	 ...
	 */
	public function get_list_store($_args = [])
	{
		if(!$this->user_id)
		{
			return false;
		...
		$result          = \lib\db\stores::search($this->user_id, $meta);
		$temp            = [];
		foreach ($result as $key => $value)
		{
			$check = $this->ready_store($value);
			if($check)
			{
				$temp[] = $check;
			}
	 ...
	 * @return     <type>  The store.
	 */
	public function get_list_store_child($_args = [])
	{
		if(!$this->user_id)
		{
			return false;
	 ...
	 * @return     <type>  The store.
	 */
	public function get_store($_options = [])
	{
		$default_options =
		[
			'debug' => true,
		...
		if(!$id && !$shortname)
		{
			if($_options['debug'])
			{
				logs::set('api:store:id:shortname:not:set', $this->user_id, $log_meta);
		...
		if($id && $shortname)
		{
			logs::set('api:store:id:shortname:together:set', $this->user_id, $log_meta);
			if($_options['debug'])
			{
		...
		if($id)
		{
			$result = \lib\db\stores::access_store_id($id, $this->user_id, ['action' => 'view']);
		}
		else
		{
			$result = \lib\db\stores::access_store($shortname, $this->user_id, ['action' => 'view']);
		}
		...
		if(!$result)
		{
			...
			if($result)
			{
				if(\lib\permission::access('load:all:store', null, $this->user_id))
				{
					$result = $result;
				}
				else
		...
		if(!$result)
		{
			logs::set('api:store:access:denide', $this->user_id, $log_meta);
			if($_options['debug'])
			{
		...
		$result = $this->ready_store($result);

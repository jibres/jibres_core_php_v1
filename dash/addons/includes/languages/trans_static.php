<?php
class TransStatic
{
 private function trans_static()
 {

	echo T_("Dash");
	echo T_("dash");
	echo T_("Powered by Dash");
	echo T_("Javad Evazzadeh");
	echo T_("Hasan Salehi");
	echo T_("Ahmad Karimi");
	echo T_("Reza Mohiti");
	echo T_("Another Project with Dash");
	echo T_("Dash is an artichokes for PHP programming!!");
	echo T_("Dash is powerfull.");
	echo T_("Insert Successfully");
	echo T_("Insert Failed!");
	echo T_("Update Successfully");
	echo T_("Update Failed!");
	echo T_("Delete Successfully");
	echo T_("Delete Failed!");
	echo T_("Transaction error");
	echo T_("Transaction Error");

	// -------------------------------------------------------- Common Words
	echo T_("Without mobile");

	// -------------------------------------- date
	echo T_("year");
	echo T_("month");
	echo T_("week");
	echo T_("day");
	echo T_("hour");
	echo T_("minute");
	echo T_("second");
	echo T_("ago");
	echo T_(",");
	echo T_(" ,");
	echo T_(", ");
	echo T_("s ");
	echo T_("A few seconds ago");

	// -------------------------------------- file
	echo T_("item");
	echo T_("empty");
	echo T_("NaN");
	echo T_("Byte");
	echo T_("KB");
	echo T_("MB");
	echo T_("GB");
	echo T_("TB");
	echo T_("PB");


	echo T_("id does not exist!");
	echo T_("all require fields must fill");
	echo T_("some fields must be change for update!");
	echo T_("Ermile");
	echo T_("ermile");
	echo T_("Made in IRAN");
	echo T_("Iran");
	echo T_("iran");
	echo T_("Qom");
	echo T_("qom");
	echo T_("submit");
	echo T_("save");
	echo T_("add new");
	echo T_("signin");
	echo T_("sign in");
	echo T_("signup");
	echo T_("sign up");
	echo T_("register");
	echo T_("home");
	echo T_("Home");
	echo T_("homepage");
	echo T_("Homepage");
	echo T_("admin");
	echo T_("error in field");

	echo T_("cp");
	echo T_("Cp");
	echo T_("account");
	echo T_("SiteContent");
	echo T_("Options");
	echo T_("Back to");
	echo T_("Actions");
	echo T_("Add New Record");
	echo T_("delete record");
	echo T_("seriously, are you sure? There's no coming back.");
	echo T_("Cancel");
	echo T_("or");
	echo T_("Delete");
	echo T_("Dashboard");
	echo T_("Posts");
	echo T_("Pages");
	echo T_("Terms");
	echo T_("Attachments");
	echo T_("Comments");
	echo T_("Visitors");
	echo T_("Users");
	echo T_("My Profile");
	echo T_("Profile");
	echo T_("Lock Screen");
	echo T_("Logout");
	echo T_("Powered by Dash.");
	echo T_("Powered by Ermile.");
	echo T_("All right reserved.");
	echo T_("Version");
	echo T_("Comming Soon");
	echo T_("Fantastic");
	echo T_("Copy");
	echo T_("Search");
	echo T_("Cut");
	echo T_("Successfully");
	echo T_("Key");
	echo T_("Value");

	echo T_("Login Successfully");
	echo T_("Login failed!");

	// -------------------------------------------------------- Public Sentences

	echo T_("Mobile or password is incorrect");
	echo T_("Please forward this message to administrator");
	echo T_("Please enter title");
	echo T_("Assign keywords to your posts using tags");
	echo T_("Use categories to define sections of your site and group related posts");
	echo T_("Use categories to define sections of your site and group related files");
	echo T_("Use posts to share your news in specefic category");
	echo T_("Use pages to share your static content");
	echo T_("Upload your media");
	echo T_("page");
	echo T_("If you want to change password enter it, else leave it blank");
	echo T_("Enter password within 5 to 40 character");
	echo T_("post");
	echo T_("tag");
	echo T_("category");
	echo T_("filecategory");
	echo T_("File category");
	echo T_("File Category");
	echo T_("Filecategories");
	echo T_("attachment");
	echo T_("option");
	echo T_("search");
	echo T_("upload");
	echo T_("View");
	echo T_("Add");
	echo T_("Edit");
	echo T_("Control Panel");
	echo T_("Update");
	echo T_("visitor");
	echo T_("Editor");
	echo T_("editor");
	echo T_("Admin");
	echo T_("admin");
	echo T_("Administrator");
	echo T_("Writer");
	echo T_("utility");
	echo T_("Utility");
	echo T_("Tools");
	echo T_("tools");
	echo T_("tool");
	echo T_("Chart must be contain at least 2 column!");
	echo T_("Data not exist!");

	echo T_("Mobile");
	echo T_("Password");
	echo T_("between 5-40 character");
	echo T_("between 5-40 character. be tricky!");
	echo T_("Login");
	echo T_("Create an account");
	echo T_("Change password");
	echo T_("SMS Delivery");
	echo T_("SMS Callback");
	echo T_("Ermile");
	echo T_("edit");
	echo T_("book");
	echo T_("books");
	echo T_("Book Categories");
	echo T_("Bookcategories");
	echo T_("bookcategory");
	echo T_("Use categories to define sections of your site and group related books");
	echo T_("Use book to define important parts to use in posts");
	echo T_("Duplicate - File exist");
	echo T_("Fail on tranfering file");
	echo T_("Error");
	echo T_("Invalid parameters");
	echo T_("No file sent");
	echo T_("Exceeded filesize limit");
	echo T_("Unknown errors");
	echo T_("We don't support this type of file");

	echo T_("Publish new post in social networks");
	echo T_("Socialnetwork");
	echo T_("socialnetwork");
	echo T_("Page Views");
	echo T_("Description");
	echo T_("Title");
	echo T_("Date");
	echo T_("Size");
	echo T_("Ext");

	echo T_("pages");
	echo T_("tags");
	echo T_("categories");
	echo T_("filecategories");
	echo T_("bookcategories");
	echo T_("permissions");
	echo T_("Permissions");
	echo T_("logout");
	echo T_("lock");
	echo T_("profile");

	// -------------------------------------------------------- CP

	// -------------------------------------- Permissions
	echo T_("You can't view this part of system");
	echo T_("You can't add new");
	echo T_("you can't edit");
	echo T_("You can't delete");
	echo T_("You can't access to this part of system");
	echo T_("Because of your permission");
	echo T_("You can't access to this page!");

	// -------------------------------------- Options
	// ----------------------------------- General
	echo T_("Site");
	echo T_("title");
	echo T_("enable");
	echo T_("disable");
	echo T_("help");
	echo T_("For multilanguage sites enter title in English and translate it");


	// ----------------------------------- Config
	echo T_("main URL");
	echo T_("Enable coming soon");
	echo T_("debug mode status");
	echo T_("Save as cookie");
	echo T_("Log visitors");
	echo T_("Use main account");
	echo T_("Main account");
	echo T_("Default Language");
	echo T_("Feature status");
	echo T_("Use fake subdomain");
	echo T_("Use SMS service");
	echo T_("Use social networks");
	echo T_("Use account");


	// ----------------------------------- Social
	echo T_("line number");
	echo T_("apikey");
	echo T_("Github");
	echo T_("Linkedin");
	echo T_("Aparat");
	echo T_("Google Plus");
	echo T_("Status of sms service");
	echo T_("Kavenegar");
	echo T_("SMS service");
	echo T_("service");
	echo T_("ConsumerSecret");
	echo T_("AccessToken");
	echo T_("AccessTokenSecret");
	echo T_("app_id");
	echo T_("app_secret");
	echo T_("redirect_url");
	echo T_("required_scope");
	echo T_("page_id");
	echo T_("access_token");
	echo T_("client_token");
	echo T_("Send message for");
	echo T_("verification");
	echo T_("recovery");
	echo T_("changepass");
	echo T_("Regional restriction");
	echo T_("Message header");
	echo T_("Message footer");
	echo T_("Force one message");
	echo T_("We can't give service to this number");
	echo T_("Simulate SMS (Debugging)");
	echo T_("Please set apikey and linenumber");
	echo T_("send");
	echo T_(" to ");
	echo T_("until");
	echo T_("Recovery account");
	echo T_("Send message in custom situation");
	echo T_("Message detail");
	echo T_("SMS api detail");
	echo T_("Status of twitter sharing");
	echo T_("Status of facebook sharing");
	echo T_("Status of telegram sharing");


	// ----------------------------------- Account
	echo T_("Allow registration");
	echo T_("After login redirect to");
	echo T_("Default permission");
	echo T_("Account Status");
	echo T_("Access with pass phrase");
	echo T_("Pass phrase key");
	echo T_("Pass phrase value");
	echo T_("Allow recovery account");
	echo T_("Status of account service");
	echo T_("Increase account security");
	echo T_("Signup Settings");



	// -------------------------------------------------------- Account

	// -------------------------------------- Login
	echo T_("Enter your registered mobile");
	echo T_("Enter your password");

	// -------------------------------------- Signup
	echo T_("Mobile number exist!");
	echo T_("Register successfully");
	echo T_("Register failed!");
	echo T_("Please forward this message to administrator");

	// -------------------------------------- Recovery
	echo T_("Recovery");
	echo T_("your recovery code is");
	echo T_("recovery failed!");
	echo T_("Mobile number is incorrect");

	// -------------------------------------- Verification
	echo T_("Verificate");
	echo T_("Verification");
	echo T_("verify successfully.");
	echo T_("please Input your new password");
	echo T_("verify failed!");
	echo T_("this data is incorrect");
	echo T_("Check your mobile and enter the code");

	// -------------------------------------- ChangePass
	echo T_("Change Password");
	echo T_("Current Password");
	echo T_("New Password");
	echo T_("Change it");
	echo T_("change password successfully");
	echo T_("change password failed!");

	// -------------------------------------- Sessions
	echo T_("Windows");
	echo T_("Linux");
	echo T_("Mac");
	echo T_("Chrome");
	echo T_("Gecko");


	// -------------------------------------------------------- Permissions
	echo T_("First you must enter name of permission");
	echo T_("This permission name exist!");
	echo T_("You can edit this permission");
	echo T_("Define or edit user permissions to allow or block access to special pages");
	echo T_("You are editing permission");


	// -------------------------------------------------------- SMS
	echo T_("We send a verification code for you");
	echo T_("We send a verification code to this number");
	echo T_("You account is verified successfully");
	echo T_("Your verification code is");
	echo T_("Your password is changed successfully");
	echo T_("Now we only support IRAN!");

	echo T_("Thanks for using our service");


	// -------------------------------------------------------- Telegram messages
	// -------------------------------------- Register
	echo T_("Dear user");
	echo T_("For using this service we need to register your phone number.");
	echo T_("Please send your number with below keyboard to complete registeration.");
	echo T_("Dear friend");
	echo T_("We need your phone number to verifying your account.");
	echo T_("If you dont want share your number, we cant presenting our services to you!");
	echo T_("Registering user is successful.");
	echo T_("Cancel registration and return to main menu");
	echo T_("Please only use below keyboard");
	echo T_("We need your contact!");
	// -------------------------------------- Profile Menu
	echo T_("Profile");
	echo T_("Register User 📱");
	echo T_("Register User");
	echo T_("Return");
	echo T_("Register Mobile Number 📱");
	echo T_("Register Mobile Number");
	echo T_("Register Address");
	echo T_("By completing your profile, help us to present better service.");
	echo T_("We will thank you for this.");
	// -------------------------------------- Save user contact
	echo T_("Your phone number registered successfully;)");
	echo T_("We dont need another users contact:?)");
	echo T_("We need mobile number!");
	echo T_(" ");
	// -------------------------------------- users table fields
	echo T_("table");
	echo T_("users");
	echo T_("id");
	echo T_("username");
	echo T_("displayname");
	echo T_("gender");
	echo T_("title");
	echo T_("password");
	echo T_("mobile");
	echo T_("email");
	echo T_("chatid");
	echo T_("status");
	echo T_("avatar");
	echo T_("parent");
	echo T_("permission");
	echo T_("type");
	echo T_("datecreated");
	echo T_("datemodified");
	echo T_("pin");
	echo T_("ref");
	echo T_("twostep");
	echo T_("birthday");
	echo T_("unit_id");
	echo T_("language");
	echo T_("meta");
	echo T_("website");
	echo T_("facebook");
	echo T_("twitter");
	echo T_("instagram");
	echo T_("linkedin");
	echo T_("gmail");
	echo T_("sidebar");
	echo T_("firstname");
	echo T_("lastname");
	echo T_("bio");



	// -------------------------------------------- PERMISSION
	echo T_("cpPermissionAdd");
	echo T_("cpPermissionEdit");
	echo T_("cpPermissionDelete");
	echo T_("cpPermissionView");
	echo T_("cpCommentsEdit");
	echo T_("cpCommentsView");
	echo T_("contentCp");
	echo T_("cpUsersView");
	echo T_("cpUsersEdit");
	echo T_("cpUsersAdd");
	echo T_("cpCategoryView");
	echo T_("cpTagView");
	echo T_("cpCategoryEdit");
	echo T_("cpTagEdit");
	echo T_("cpCategoryAdd");
	echo T_("cpTagAdd");
	echo T_("cpBackup");
	echo T_("cpPageAdd");
	echo T_("cpPostsAdd");
	echo T_("cpPageEdit");
	echo T_("cpPostsEdit");
	echo T_("cpPageView");
	echo T_("cpPostsView");
	echo T_("cpHelpView");
	echo T_("cpHelpsView");
	echo T_("cpHelpEdit");
	echo T_("cpHelpsEdit");

	// ------------------------------------------ PERMISSION GROUP
	echo T_("Admin");
	echo T_("Editor");
	echo T_("Writer");


	// -------------------------------------- iran province list
	echo T_("east azerbaijan");
	echo T_("west azerbaijan");
	echo T_("ardabil");
	echo T_("isfahan");
	echo T_("alborz");
	echo T_("ilam");
	echo T_("bushehr");
	echo T_("tehran");
	echo T_("Asia/Tehran");
	echo T_("chaharmahal and bakhtiari");
	echo T_("south khorasan");
	echo T_("razavi khorasan");
	echo T_("north khorasan");
	echo T_("khuzestan");
	echo T_("zanjan");
	echo T_("semnan");
	echo T_("sistan and baluchestan");
	echo T_("fars");
	echo T_("qazvin");
	echo T_("qom");
	echo T_("kordestan");
	echo T_("kerman");
	echo T_("kermanshah");
	echo T_("kohgiluyeh and boyerahmad");
	echo T_("golestan");
	echo T_("gilan");
	echo T_("lorestan");
	echo T_("mazandaran");
	echo T_("markazi");
	echo T_("hormozgan");
	echo T_("hamedan");
	echo T_("yazd");



	echo T_("Saderat");
	echo T_("Mellat");
	echo T_("Tejarat");
	echo T_("Melli");
	echo T_("Sepah");
	echo T_("Keshavarzi");
	echo T_("Maskan");
	echo T_("Refah");
	echo T_("Novin");
	echo T_("Ansar");
	echo T_("Pasargad");
	echo T_("Saman");
	echo T_("Sina");
	echo T_("Post");
	echo T_("Ghavamin");
	echo T_("Taavon");
	echo T_("Shahr");
	echo T_("Ayande");
	echo T_("Sarmayeh");
	echo T_("Day");
	echo T_("Hekmat");
	echo T_("Iranzamin");
	echo T_("Karafarin");
	echo T_("Gardeshgari");
	echo T_("Madan");
	echo T_("Tsaderat");
	echo T_("Khavarmiyane");
	echo T_("Ivbb");
	echo T_("Irkish");
	echo T_("Asanpardakht");
	echo T_("Zarinpal");
	echo T_("Payir");

	// -------------------------------------- headers
	echo T_('Continue');
	echo T_('Switching Protocols');
	echo T_('Processing');
	echo T_('OK');
	echo T_('Created');
	echo T_('Accepted');
	echo T_('Non-Authoritative Information');
	echo T_('No Content');
	echo T_('Reset Content');
	echo T_('Partial Content');
	echo T_('Multi-Status');
	echo T_('IM Used');
	echo T_('Multiple Choices');
	echo T_('Moved Permanently');
	echo T_('Found');
	echo T_('See Other');
	echo T_('Not Modified');
	echo T_('Use Proxy');
	echo T_('Reserved');
	echo T_('Temporary Redirect');
	echo T_('Permanent Redirect');
	echo T_('Bad Request');
	echo T_('Unauthorized');
	echo T_('Payment Required');
	echo T_('Forbidden');
	echo T_('Not Found');
	echo T_('Method Not Allowed');
	echo T_('Not Acceptable');
	echo T_('Proxy Authentication Required');
	echo T_('Request Timeout');
	echo T_('Conflict');
	echo T_('Gone');
	echo T_('Length Required');
	echo T_('Precondition Failed');
	echo T_('Request Entity Too Large');
	echo T_('Request-URI Too Long');
	echo T_('Unsupported Media Type');
	echo T_('Requested Range Not Satisfiable');
	echo T_('Expectation Failed');
	echo T_('I\'m a teapot');
	echo T_('Misdirected Request');
	echo T_('Unprocessable Entity');
	echo T_('Locked');
	echo T_('Failed Dependency');
	echo T_('Upgrade Required');
	echo T_('Precondition Required');
	echo T_('Too Many Requests');
	echo T_('Request Header Fields Too Large');
	echo T_('Unavailable For Legal Reasons');
	echo T_('Internal Server Error');
	echo T_('Not Implemented');
	echo T_('Bad Gateway');
	echo T_('Service Unavailable');
	echo T_('Gateway Timeout');
	echo T_('HTTP Version Not Supported');
	echo T_('Variant Also Negotiates');
	echo T_('Insufficient Storage');
	echo T_('Not Extended');
	echo T_('Network Authentication Required');



	//---------------------------------------- COUNTRY LIST ----------------------------
	echo T_("afghanistan");
	echo T_("åland");
	echo T_("albania");
	echo T_("algeria");
	echo T_("american samoa");
	echo T_("andorra");
	echo T_("angola");
	echo T_("anguilla");
	echo T_("antarctica");
	echo T_("antigua and barbuda");
	echo T_("argentina");
	echo T_("armenia");
	echo T_("aruba");
	echo T_("australia");
	echo T_("austria");
	echo T_("azerbaijan");
	echo T_("bahamas");
	echo T_("bahrain");
	echo T_("bangladesh");
	echo T_("barbados");
	echo T_("belarus");
	echo T_("belgium");
	echo T_("belize");
	echo T_("benin");
	echo T_("bermuda");
	echo T_("bhutan");
	echo T_("bolivia");
	echo T_("bonaire");
	echo T_("bosnia and herzegovina");
	echo T_("botswana");
	echo T_("bouvet island");
	echo T_("brazil");
	echo T_("british indian ocean territory");
	echo T_("british virgin islands");
	echo T_("brunei");
	echo T_("bulgaria");
	echo T_("burkina faso");
	echo T_("burundi");
	echo T_("cambodia");
	echo T_("cameroon");
	echo T_("canada");
	echo T_("cape verde");
	echo T_("cayman islands");
	echo T_("central african republic");
	echo T_("chad");
	echo T_("chile");
	echo T_("china");
	echo T_("christmas island");
	echo T_("cocos (keeling) islands");
	echo T_("colombia");
	echo T_("comoros");
	echo T_("cook islands");
	echo T_("costa rica");
	echo T_("croatia");
	echo T_("cuba");
	echo T_("curacao");
	echo T_("cyprus");
	echo T_("czech republic");
	echo T_("democratic republic of the congo");
	echo T_("denmark");
	echo T_("djibouti");
	echo T_("dominica");
	echo T_("dominican republic");
	echo T_("east timor");
	echo T_("ecuador");
	echo T_("egypt");
	echo T_("el salvador");
	echo T_("equatorial guinea");
	echo T_("eritrea");
	echo T_("estonia");
	echo T_("ethiopia");
	echo T_("falkland islands");
	echo T_("faroe islands");
	echo T_("fiji");
	echo T_("finland");
	echo T_("france");
	echo T_("french guiana");
	echo T_("french polynesia");
	echo T_("french southern territories");
	echo T_("gabon");
	echo T_("gambia");
	echo T_("georgia");
	echo T_("germany");
	echo T_("ghana");
	echo T_("gibraltar");
	echo T_("greece");
	echo T_("greenland");
	echo T_("grenada");
	echo T_("guadeloupe");
	echo T_("guam");
	echo T_("guatemala");
	echo T_("guernsey");
	echo T_("guinea");
	echo T_("guinea-bissau");
	echo T_("guyana");
	echo T_("haiti");
	echo T_("heard island and mcdonald islands");
	echo T_("honduras");
	echo T_("hong kong");
	echo T_("hungary");
	echo T_("iceland");
	echo T_("india");
	echo T_("indonesia");
	echo T_("iraq");
	echo T_("ireland");
	echo T_("isle of man");
	echo T_("italy");
	echo T_("ivory coast");
	echo T_("jamaica");
	echo T_("japan");
	echo T_("jersey");
	echo T_("jordan");
	echo T_("kazakhstan");
	echo T_("kenya");
	echo T_("kiribati");
	echo T_("kosovo");
	echo T_("kuwait");
	echo T_("kyrgyzstan");
	echo T_("laos");
	echo T_("latvia");
	echo T_("lebanon");
	echo T_("lesotho");
	echo T_("liberia");
	echo T_("libya");
	echo T_("liechtenstein");
	echo T_("lithuania");
	echo T_("luxembourg");
	echo T_("macau");
	echo T_("macedonia");
	echo T_("madagascar");
	echo T_("malawi");
	echo T_("malaysia");
	echo T_("maldives");
	echo T_("mali");
	echo T_("malta");
	echo T_("marshall islands");
	echo T_("martinique");
	echo T_("mauritania");
	echo T_("mauritius");
	echo T_("mayotte");
	echo T_("mexico");
	echo T_("micronesia");
	echo T_("moldova");
	echo T_("monaco");
	echo T_("mongolia");
	echo T_("montenegro");
	echo T_("montserrat");
	echo T_("morocco");
	echo T_("mozambique");
	echo T_("myanmar (burma)");
	echo T_("namibia");
	echo T_("nauru");
	echo T_("nepal");
	echo T_("netherlands");
	echo T_("new caledonia");
	echo T_("new zealand");
	echo T_("nicaragua");
	echo T_("niger");
	echo T_("nigeria");
	echo T_("niue");
	echo T_("norfolk island");
	echo T_("north korea");
	echo T_("northern mariana islands");
	echo T_("norway");
	echo T_("oman");
	echo T_("pakistan");
	echo T_("palau");
	echo T_("palestine");
	echo T_("panama");
	echo T_("papua new guinea");
	echo T_("paraguay");
	echo T_("peru");
	echo T_("philippines");
	echo T_("pitcairn islands");
	echo T_("poland");
	echo T_("portugal");
	echo T_("puerto rico");
	echo T_("qatar");
	echo T_("republic of the congo");
	echo T_("réunion");
	echo T_("romania");
	echo T_("russia");
	echo T_("rwanda");
	echo T_("saint barthélemy");
	echo T_("saint helena");
	echo T_("saint kitts and nevis");
	echo T_("saint lucia");
	echo T_("saint martin");
	echo T_("saint pierre and miquelon");
	echo T_("saint vincent and the grenadines");
	echo T_("samoa");
	echo T_("san marino");
	echo T_("são tomé and príncipe");
	echo T_("saudi arabia");
	echo T_("senegal");
	echo T_("serbia");
	echo T_("seychelles");
	echo T_("sierra leone");
	echo T_("singapore");
	echo T_("sint maarten");
	echo T_("slovakia");
	echo T_("slovenia");
	echo T_("solomon islands");
	echo T_("somalia");
	echo T_("south africa");
	echo T_("south georgia and the south sandwich islands");
	echo T_("south korea");
	echo T_("south sudan");
	echo T_("spain");
	echo T_("sri lanka");
	echo T_("sudan");
	echo T_("suriname");
	echo T_("svalbard and jan mayen");
	echo T_("swaziland");
	echo T_("sweden");
	echo T_("switzerland");
	echo T_("syria");
	echo T_("taiwan");
	echo T_("tajikistan");
	echo T_("tanzania");
	echo T_("thailand");
	echo T_("togo");
	echo T_("tokelau");
	echo T_("tonga");
	echo T_("trinidad and tobago");
	echo T_("tunisia");
	echo T_("turkey");
	echo T_("turkmenistan");
	echo T_("turks and caicos islands");
	echo T_("tuvalu");
	echo T_("minor outlying islands");
	echo T_("uganda");
	echo T_("ukraine");
	echo T_("united arab emirates");
	echo T_("united kingdom");
	echo T_("united states");
	echo T_("uruguay");
	echo T_("uzbekistan");
	echo T_("vanuatu");
	echo T_("vatican city");
	echo T_("venezuela");
	echo T_("vietnam");
	echo T_("wallis and futuna");
	echo T_("western sahara");
	echo T_("yemen");
	echo T_("zambia");
	echo T_("zimbabwe");



	echo T_("enable");
	echo T_("disable");
	echo T_("expire");
	echo T_("deleted");
	echo T_("lock");
	echo T_("awaiting");
	echo T_("block");
	echo T_("filter");
	echo T_("close");
	echo T_("male");
	echo T_("female");
	echo T_("single");
	echo T_("married");
	echo T_("leave");
	echo T_("real");
	echo T_("learn");
	echo T_("requirement");
	echo T_("beside");
	echo T_("soon");
	echo T_("next");
	echo T_("active");
	echo T_("deactive");
	echo T_("full");
	echo T_("all");
	echo T_("sunday");
	echo T_("monday");
	echo T_("tuesday");
	echo T_("wednesday");
	echo T_("thursday");
	echo T_("friday");
	echo T_("saturday");
	echo T_("class");
	echo T_("exam");
	echo T_("weekly");
	echo T_("rotational");
	echo T_("date");
	echo T_("taken");
	echo T_("remove");
	echo T_("add");
	echo T_("removeforce");
	echo T_("removeadmin");
	echo T_("cancel");
	echo T_("student");
	echo T_("expert");
	echo T_("acept");
	echo T_("reject");
	echo T_("discount");
	echo T_("Count");
	echo T_("Average");
	echo T_("teacher");
	echo T_("other");
	echo T_("removed");
	echo T_("unreachable");
	echo T_("paid");
	echo T_("skipped");
	echo T_("critical");
	echo T_("high");
	echo T_("medium");
	echo T_("low");
	echo T_("deliver");
	echo T_("draft");
	echo T_("true");
	echo T_("false");
	echo T_("skip");
	echo T_("gift");
	echo T_("prize");
	echo T_("transfer");
	echo T_("promo");
	echo T_("money");
	echo T_("expired");
	echo T_("filtered");
	echo T_("blocked");
	echo T_("spam");
	echo T_("request");
	echo T_("redirect");
	echo T_("pending");
	echo T_("error");
	echo T_("verify_request");
	echo T_("verify_error");
	echo T_("ok");
	echo T_("awaiting");
	echo T_("approved");
	echo T_("unapproved");
	echo T_("Ok");
	echo T_("Awaiting");
	echo T_("Approved");
	echo T_("Unapproved");

	echo T_("SMS");
	echo T_("Email");
	echo T_("cpSmsSend");
	echo T_("cpSmsTemplateView");
	echo T_("cpSmsTemplateEdit");
	echo T_("cpSmsSendGroup");
	echo T_("cpSMS");
	echo T_("cpEmailSend");
	echo T_("cpEmail");

	echo T_("cpPostsViewAll");
	echo T_("cpPostsEditStatus");
	echo T_("cpPostsEditPublished");
	echo T_("cpPostsEditForOthers");
	echo T_("cpPostsDelete");
	echo T_("cpPostsDeleteForOthers");
	echo T_("cpCategoryDelete");
	echo T_("cpTagDelete");
	echo T_("cpPageDelete");
	echo T_("cpAttachmentView");
	echo T_("cpAttachmentAdd");
	echo T_("cpAttachmentEdit");
	echo T_("cpAttachmentDelete");
	echo T_("cpCommentsDelete");
	echo T_("cpUsersPermission");
	echo T_("cpUsersPasswordChange");
	echo T_("cpUsersDelete");
	echo T_("cpSmsTemplateDelete");

	echo T_("Control Panel CMS");
	echo T_("Control Panel Users");
	echo T_("Control Panel Contact");

	echo T_("Posts");
	echo T_("Categories");
	echo T_("Tags");
	echo T_("Pages");
	echo T_("Attachments");
	echo T_("Comments");
	echo T_("Backup");
	echo T_("Users");
	echo T_("Permissions");
	echo T_("SMS");
	echo T_("Email");

	echo T_("cpContactView");
	echo T_("cpContactChangeStatus");
	echo T_("cpTemplateSmsView");
	echo T_("answered");
	echo T_("Answered");

	echo T_("cpSupportTagView");
	echo T_("SupportTags");
	echo T_("cpSupportTagAdd");
	echo T_("cpSupportTagEdit");
	echo T_("cpSupportTagDelete");
	echo T_("contentSupport");
	echo T_("supportTagView");
	echo T_("Support Panel");
	echo T_("supportTagAdd");
	echo T_("supportTagEdit");
	echo T_("supportTagDelete");
	echo T_("supportDeleteTicket");
	echo T_("supportTicketAnswer");
	echo T_("supportTicketAddTag");
	echo T_("supportTicketAnswerOtherSubdomain");
	echo T_("supportTicketChangeStatus");
	echo T_(":displayname add new ticket in :domain");

	echo T_("load su whit user :user");
	echo T_("The :user was loaded su content");
	echo T_("We Noticed a New Login, :displayname");
	echo T_("If this was you, you can safely disregard this message. If you didn't do this, please change your password to help secure your account. Learn more about keeping your account secure.");

	echo T_("You have new ticket");
	echo T_("You have new ticket from :displayname \n see :link");


	echo T_("cpTagHelpView");
	echo T_("cpTagHelpAdd");
	echo T_("cpTagHelpEdit");
	echo T_("cpTagHelpDelete");
	echo T_("cpTagSupportView");
	echo T_("cpTagSupportAdd");
	echo T_("cpTagSupportEdit");
	echo T_("cpTagSupportDelete");
	echo T_("cpHelpCenterView");
	echo T_("cpHelpCenterViewAll");
	echo T_("cpHelpCenterAdd");
	echo T_("cpHelpCenterEdit");
	echo T_("cpHelpCenterEditStatus");
	echo T_("cpHelpCenterEditPublished");
	echo T_("cpHelpCenterEditForOthers");
	echo T_("cpHelpCenterDelete");
	echo T_("cpHelpCenterDeleteForOthers");
	echo T_("cpEmailTemplateView");
	echo T_("cpEmailTemplateEdit");
	echo T_("supportTicketReport");
	echo T_("supportTicketShowLog");
	echo T_("supportTicketManage");
	echo T_("supportTicketManageSubdomain");
	echo T_("supportTicketAnswer");
	echo T_("supportEditMessage");
	echo T_("supportDeleteMessage");
	echo T_("supportTicketShowMobile");
	echo T_("supportTicketAssignTag");
	echo T_("supportTicketAddNote");
	echo T_("supportTicketSignature");
	echo T_("supportTicketClose");
	echo T_("supportTicketReOpen");
	echo T_("supportTicketDelete");
	echo T_("supportTicketShowLastVisitor");
	echo T_("cpVisitorView");

	echo T_("Send notification");
	echo T_("Set ticket close if solved");

	echo T_("Nginx service restarted");
	echo T_("Mysql service restarted");
	echo T_("Sshpass installed");
	echo T_("Invalid command sended");
	echo T_("Cronjob detail changed");
	echo T_("Disable cronjob");
	echo T_("Database upgraded to new version");
	echo T_("Git config is modified");
	echo T_("Generate sitemap");
	echo T_("Run translation extract");
	echo T_("Git update");
	echo T_("Telegram send message");
	echo T_("Telegram send photo");
	echo T_("Telegram set webhook");
	echo T_("Add transaction manually");
	echo T_("The title :name");
	echo T_("Load su whit :user");
	echo T_("You have new ticket");
	echo T_("We Noticed a New Login, :displayname");
	echo T_("Send message :msg to chatid :chatid");
	echo T_("Send photo to chatid :chatid");
	echo T_(":url \n whit max connetcion :max_connections");
	echo T_("Add transaction :title whit :plus plus and :minus minus to :user_id");
	echo T_("The :user content");
	echo T_("The :user was loaded su content");
	echo T_("You have new ticket from :displayname");
	echo T_("If this was you, you can safely disregard this message. If you didn't do this, please change your password to help secure your account. Learn more about keeping your account secure.");



	echo T_("Add new ticket");
	echo T_("The ticket closed");
	echo T_("The ticket set as open ticket");
	echo T_("The ticket delete ticket");
	echo T_("The ticket set as solved ticket");
	echo T_("The ticket set as unsolved ticket");
	echo T_("Answer ticket");
	echo T_("Add new message to ticket");
	echo T_("Add new note to ticket");
	echo T_("The message was edited");
	echo T_(";displayname add new ticket");
	echo T_(";displayname close the ticket");
	echo T_(";displayname re open the ticket");
	echo T_(";displayname delete the ticket");
	echo T_(";displayname set as solved the ticket");
	echo T_(";displayname set as unsolved the ticket");
	echo T_(";displayname answer ticket");
	echo T_(";displayname add new message to ticket");
	echo T_(";displayname add new note to ticket");
	echo T_("New Ticket");
	echo T_("Close the ticket");
	echo T_("Open the ticket");
	echo T_("Deleted");
	echo T_("Solved");
	echo T_("Back to unsolved");
	echo T_("Remove temp csv file");

	echo T_("contentCrm");
	echo T_("CRM");
	echo T_("CP");
	echo T_("Customer relationship management");

	echo T_("supportTicketTrash");
	echo T_("cpSupportTagAdd");

	echo T_("Invoices");
	echo T_("Userdetail");
	echo T_("Sessions");

	echo T_("Panel");
	echo T_("panel");

	echo T_("activeuser");
	echo T_("deactiveuser");
	echo T_("user_awaiting");
	echo T_("user_removed");
	echo T_("user_filter");
	echo T_("news");


	echo T_("cpMagAdd");
	echo T_("cpMagEditPublished");
	echo T_("cpMagEditForOthers");
	echo T_("cpMagDelete");
	echo T_("cpMagDeleteForOthers");
	echo T_("cpMagEditStatus");
	echo T_("cpDayEvent");
	echo T_("cpContactView");
	echo T_("cpMagView");
	echo T_("cpMagEdit");

	echo T_("cpChangePostCreator");
	echo T_("Contributor");
 }
}
?>
 ...
 * @param [type] $_msgid [description]
 */
function T_($_msgid, $_replace = null)
{
	$result = null;
	if(is_string($_msgid) && $_msgid)
	{
		if(_check_locale_and_function())
		{
			$result = _($_msgid);
	...
	elseif(is_array($_msgid))
	{
		$result = $_msgid;
	}
	elseif(is_numeric($_msgid))
	{
		$result = $_msgid;
	}
	...
	if($_replace !== false && !is_array($result) && strpos($result, ':') !== false)
	{
		...
		if($_replace && is_array($_replace))
		{
			...
			{
				$replaceArray = array_merge($replaceArray, $_replace);
			}
			else
			{
				$replaceArray = $_replace;
			}
		}
		// if we collect array to replace, do replace process
		if(is_array($replaceArray))
		{

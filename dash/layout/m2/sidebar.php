...
class sidebar
{
	...
	private static function business_sidebar_menu()
	{
		...
		if(in_array($module, ['setting']) && !($content === 'a' && $module === 'setting' && in_array($child, [
					'domain', 'menu',
				])))
		{
		...
		if(in_array($content, ['site',]) || ($content === 'a' && $module === 'setting' && in_array($child, [
					'domain', 'menu',
				])))
		{
		...
		if(\dash\request::is_pwa())
		{
			...
			$menu['website'] =
				[
					'title'     => T_("View your online store"),
					'url'       => \lib\store::url(),
					'icon'      => 'heart',
					'iconColor' => '#a1b2c3',
					'class'     => null,

...
class socialnetwork
{
	...
	{
		$count_socialnetwork = \lib\db\setting\get::count_platform_cat_key('website', 'homepage', 'body_line_socialnetwork');
		$count_socialnetwork = intval($count_socialnetwork) + 1;

		return T_("Socialnetwork"). ' '. \dash\fit::number($count_socialnetwork);
	}
	...
	public static function get($_id)
	{
		...
		if(isset($result['type']) && $result['type'] === 'socialnetwork')
		{
			// ok
		}
		else
		...
		if(isset($result['socialnetwork']) && is_array($result['socialnetwork']))
		{
			$result['socialnetwork'] = self::ready($result['socialnetwork']);
		}
	...
	public static function add($_args)
	{
		...
		$line_id = \lib\app\website\body\add::line('socialnetwork', ['title' => $data['title'], 'publish' => $data['publish']]);
		...
		\dash\notif::ok(T_("Socialnetwork added"));

		// retrun id to redirect to this socialnetwork
	...
	public static function edit($_args, $_line_id)
	{
		...
		$line_option = \lib\app\website\body\template::get('socialnetwork');
		...
		if(!$saved_value || !isset($saved_value['socialnetwork']))
		{
			return false;
		}
		...
		\dash\notif::ok(T_("Socialnetwork edited"));

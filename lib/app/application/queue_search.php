...
class queue_search
{
	...
	public static function list($_query_string, $_args)
	{
		...
		if($data['status'])
		{
			...
			$and[]                      = " store_app.status = '$data[status]' ";
			self::$filter_args[T_("Type")] = $data['status'];
		...
		if(!$order_sort)
		{
			$order_sort = " ORDER BY store_app.id DESC";
		}
	...
	{
		$result = \lib\db\store_app\get::group_by_status();
		return $result;
	}
}
?>
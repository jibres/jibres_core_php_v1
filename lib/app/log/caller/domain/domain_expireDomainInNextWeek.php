...
class domain_expireDomainInNextWeek
{
	...
	public static function get_msg($_args = [])
	{
		...
		{
			$msg = T_("Domain :domain will be expire in next week", ['domain' => $domains_list[0]['domain']]);
		}
		elseif(count($domains_list) < 10)
		{
			$domain = array_column($domains_list, 'domain');
			$domain = implode(',', $domain);
			$msg = T_("Domain :domain will be expire in next week", ['domain' => $domain]);
		}
		else
		{
			$msg = T_(":count Domains will be expire in next week", ['count' => \dash\fit::number(count($domains_list))]);
		}

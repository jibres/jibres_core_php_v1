...
class edit
{
	...
	public static function edit($_args, $_id, $_option = [])
	{
		...
		$load_user  = \lib\db\users\users::get_by_id($id);
		...
		// check args
		$args = \lib\app\user\check::check($id, $_option, $load_user);
		...
		if($args['email'])
		{
			$check_email_exist = \lib\db\users\users::get(['email' => $args['email'], 'limit' => 1]);
		...
		if(!empty($args))
		{
			...
			\lib\db\users\users::update($args, $id);

		}

		if(\dash\engine\process::status())
		{
			if(intval($id) === intval(\lib\user::id()))
			{
				\lib\user::refresh();
			}

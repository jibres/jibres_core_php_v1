...
class search
{
	...
	public static function list($_param, $_and, $_or, $_order_sort = null, $_meta = [])
	{
		...
		$pagination_query = "SELECT COUNT(*) AS `count` FROM store_plan_history $q[join] $q[where] ";
		...
		$query = "SELECT $q[fields] FROM store_plan_history $q[join] $q[where] $q[order] $limit ";

...
class get
{
	...
	public static function by_form_id($_form_id, $_deleted_itme = false, $_hidden_item = false)
	{
		...
		$query  =
			"SELECT * FROM form_item WHERE form_item.form_id = $_form_id $delted $hidden ORDER BY IFNULL(form_item.sort, form_item.id) ASC ";
		$result = \dash\pdo::get($query);
		return $result;
	}
	...
	public static function randomItems($_form_id, $_countRandom)
	{
		$query =
			"
				SELECT 
				...
				FROM 
				    form_item 
				WHERE 
				    form_item.form_id = :form_id AND 
				    (form_item.`require` != 1 OR form_item.`require` IS NULL )  
				ORDER BY RAND() 
				LIMIT :limit 
		";
		$param =

    ...
    <section class="box">
      ...
      <div class="body">
        ...
        <p>
          ...
          <br>
        <?php echo T_("Also you can copy all property from another category here") ?>
        <a href="<?php echo \dash\url::this(). '/clone'. \dash\request::full_get() ?>" class="link-primary"><?php echo T_("Copy from other category") ?></a>
        </p>
        <div class="row">
    ...
    <form method="post" data-patch>
      <input type="hidden" name="itemsort" value="itemsort">
        <?php foreach (\dash\data::propertyGroup() as $key => $value) {?>
          <p class="font-bold mB0-f font-16"><?php echo $key ?> <a class="text-xs" href="<?php echo \dash\url::this(). '/editgroup'. \dash\request::full_get(['group' => $key]); ?>"><?php echo T_("Edit") ?></a></p>

    ...
    <div class="action">
      <?php if(\dash\data::allMenu()) {?>
        ...
        <select name="<?php echo $box; ?>" id="idmenu<?php echo $box; ?>" class="select22">
          <?php if(a($header_detail, 'saved', $box)) {?>
            <option value="0"><?php echo T_("Without menu"); ?></option>
          <?php }else{ ?>
            <option></option>
          <?php } //endif ?>
          <?php foreach (\dash\data::allMenu() as $key => $value) {?>
            <option value="<?php echo a($value, 'key'); ?>" <?php if(a($header_detail, 'saved', $box) == a($value, 'key')) { echo 'selected'; $currentMenuID = a($value, 'id'); $currentMenuName = a($value, 'title');} ?>><?php echo a($value, 'title'); ?></option>
          <?php } //endfor ?>
        </select>

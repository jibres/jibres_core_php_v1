      ...
      <div class="f">
        ...
        <div class="cauto">
          <select class="select22 <?php if(\dash\request::get('sort') || \dash\request::get('order')) { echo 'apply'; }?>" data-link>
            <option value="<?php echo \dash\url::that(); if(\dash\request::get('q')){ echo '?q='. \dash\request::get('q');} ?>"><?php echo T_("Sort"); ?></option>
            <option value="<?php echo \dash\url::that(). '?sort=title&order=asc'; if(\dash\request::get('q')){ echo '&q='. \dash\request::get('q');} ?>" <?php if(\dash\request::get('sort') === 'title' && \dash\request::get('order') === 'asc') { echo 'selected'; } ?>><?php echo T_("Sort by Title ASC") ?></option>
            <option value="<?php echo \dash\url::that(). '?sort=title&order=desc'; if(\dash\request::get('q')){ echo '&q='. \dash\request::get('q');} ?>" <?php if(\dash\request::get('sort') === 'title' && \dash\request::get('order') === 'desc') { echo 'selected'; } ?>><?php echo T_("Sort by Title DESC") ?></option>
            </select>
          </div>
        </div>
      </div>
...
<p class="f fs14 msg warn2">
  ...
  <?php if(\dash\data::barcodeScaned()) {?>
    ...
    <a class="c" href="<?php echo \dash\url::that(); ?>/add<?php echo \dash\data::barcodeScaned(); ?>" data-shortkey="118"><?php echo T_("Add new form"); ?> <kbd>f7</kbd></a>

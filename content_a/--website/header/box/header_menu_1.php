      ...
      <?php if(\dash\data::allMenu()) {?>
        ...
        <select name="<?php echo $box; ?>" id="idmenu<?php echo $box; ?>" class="select22">
          ...
          <?php foreach (\dash\data::allMenu() as $key => $value) {?>
            <option value="<?php echo a($value, 'id'); ?>" <?php if(a($header_detail, 'saved', $box) == a($value, 'id')) { echo 'selected'; $currentMenuID = a($value, 'id'); $currentMenuName = a($value, 'title');} ?>><?php echo a($value, 'title'); ?></option>
          <?php } //endfor ?>
        </select>

...
if($myTag)
{
  ...
  foreach ($myTag as $key => $value)
  {
    if((isset($value['status']) && $value['status'] == 'enable' ) || $cpTagSupportEdit)
    {
      ...
      echo '<a class="btn rounded sm mB5 '. $meta. '" href="'. $urlThis.'?tag='. @$value['slug']. '">'. @$value['title'].'</a>';
    }
  }
  echo '</div>';
}
...
<?php $lastRecordVisitor = false; ?>
  ...
  <table class="tbl1 v5 responsive">
    <tbody>
      ...
      <tr class="<?php echo @$record['value']['rowColor']; ?> <?php if(@$record['value']['type'] === 'ticket_note') { echo  ' disabled ';} ?>" id='msg<?php echo @$record['value']['id']; ?>'>
        <td class="collapsing pRa10">
          <img src="<?php echo @$record['value']['avatar']; ?>" class="avatar mRa10" alt="<?php echo @$record['value']['displayname']; ?>">
          <span class="txtB s0 fs08"><?php echo @$record['value']['displayname']; ?></span>
          ...
          <nav class="txtRa">
            ...
            <?php if(\dash\permission::check('cpUsersView')) {?><a href="<?php echo \dash\url::kingdom(); ?>/crm/member/glance?id=<?php echo @$record['value']['user_id']; ?>" title='<?php echo T_("User Profile"); ?>'><i class="sf-user-md"></i></a><?php } ?>
          </nav>
        <?php }//endif ?>
        </td>
        ...
        <?php if(\dash\permission::check('supportEditMessage')) {?>
            ...
            <a class="fs08 mLa10" href="<?php echo \dash\url::here(); ?>/message/edit?id=<?php echo @$record['value']['id']; ?>"><?php echo T_("Edit"); ?></a>
...
<tr>
  <td class="collapsing pRa10">
      ...
      <nav class="txtRa">
        <a href="<?php echo $urlThis; ?>?user_id=<?php echo @$record['value']['user_id']; ?>" title='<?php echo T_("User tickets"); ?>'><i class="sf-question-circle"></i></a>
        <a href="<?php echo \dash\url::kingdom(); ?>/crm/member/glance?id=<?php echo @$record['value']['user_id']; ?>" title='<?php echo T_("User Profile"); ?>'><i class="sf-user-md"></i></a>
  ...
  <td class="mLa10">
    <div class="msg info2 mRa10 mB0 ovh pA5">
      <div class="txtB">
        <?php echo @$record['value']['title']; ?>
      </div>
    <br>
    <?php echo @$record['value']['content']; ?>
...
?>
  ...
  <table class="tbl1 v5 responsive">
    <tbody>
      ...
      <tr class="<?php echo @$value['rowColor']; if(isset($value['type']) && $value['type'] === 'ticket_note') { echo 'disabled ';}?>" id='msg<?php echo @$value['id']; ?>'>
        <td class="collapsing">
          <img src="<?php echo @$value['avatar']; ?>" class="avatar mRa10" alt="<?php echo @$value['displayname']; ?>">
          <span class="txtB s0 fs08"><?php echo @$value['displayname']; ?></span>
          <?php if(\dash\permission::check('supportTicketShowMobile')) {?>

          <div class="txtRa fs08"><?php echo \dash\fit::mobile(@$value['mobile']); ?></div>

          <nav class="txtRa">
            <?php if(\dash\permission::check('supportTicketManage')) {?><a href="<?php echo $urlThis; ?>?user_id=<?php echo @$value['user_id']; ?>" title='<?php echo T_("User tickets"); ?>'><i class="sf-question-circle"></i></a><?php } //endif ?>
            <?php if(\dash\permission::check('cpUsersView')) {?><a href="<?php echo \dash\url::kingdom(); ?>/crm/member/glance?id=<?php echo @$value['user_id']; ?>" title='<?php echo T_("User Profile"); ?>'><i class="sf-user-md"></i></a><?php }//endif ?>
...
<?php } //endif ?>
        ...
        <td class="pRa10">
          ...
          <div class="f mT10">
           ...
           <div class="cauto os fc-mute">
            <small title='<?php echo \dash\fit::date(@$value['datecreated']); ?>'><?php echo \dash\fit::date_human(@$value['datecreated']); ?></small>
                <?php if(\dash\permission::check('supportEditMessage')) {?>

              <a class="fs08 mLa10" href="<?php echo \dash\url::here(); ?>/message/edit?id=<?php echo @$value['id']; ?>"><?php echo T_("Edit"); ?></a>
